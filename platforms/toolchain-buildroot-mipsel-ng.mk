# This toolchain is gcc 4.9.3 on uClibc-ng 1.0.6
...
CROSS_CONFIGURATION_GCC_VERSION=4.9.3
...
TARGET_CROSS_BUILD_DIR = $(BASE_DIR)/toolchain/buildroot-2015.08-rc2
TARGET_CROSS_TOP = $(BASE_DIR)/toolchain/buildroot-mipsel-linux-2.6.36-uclibc-ng-4.9.3
...
TOOLCHAIN_SOURCE=buildroot-2015.08-rc2.tar.bz2
...
BUILDROOT-MIPSEL_SOURCE_DIR=$(SOURCE_DIR)/buildroot-mipsel-ng

BUILDROOT-MIPSEL_PATCHES=$(BUILDROOT-MIPSEL_SOURCE_DIR)/uclibc-ng-1.0.6.patch
...
$(TARGET_CROSS_TOP)/.configured: $(DL_DIR)/$(TOOLCHAIN_SOURCE) #$(OPTWARE_TOP)/platforms/toolchain-$(OPTWARE_TARGET).mk
	...
	if test -n "$(BUILDROOT-MIPSEL_PATCHES)" ; \
		then cat $(BUILDROOT-MIPSEL_PATCHES) | \
...
#	cp -f $(BUILDROOT-MIPSEL_SOURCE_DIR)/gcc-4.9.3-patches/*.patch $(TARGET_CROSS_BUILD_DIR)/package/gcc/4.9.3/
...
NATIVE_GCC_EXTRA_CONFIG_ARGS=--with-gxx-include-dir=/opt/include/c++/4.9.3 --disable-__cxa_atexit --with-gnu-ld --disable-libssp --enable-target-optspace --disable-libsanitizer --disable-tls --disable-libmudflap --enable-threads --without-isl --without-cloog --with-float=soft --disable-decimal-float --with-arch=mips32r2 --with-abi=32 --enable-shared --disable-libgomp --with-gmp=$(STAGING_PREFIX) --with-mpfr=$(STAGING_PREFIX) --with-mpc=$(STAGING_PREFIX)

NATIVE_GCC_EXTRA_PATCHES=$(wildcard $(BUILDROOT-MIPSEL_SOURCE_DIR)/gcc-4.9.3-patches/*.patch)
