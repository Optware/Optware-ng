...
GIFT_CONFFILES=$(TARGET_PREFIX)/share/giFT/giftd.conf $(TARGET_PREFIX)/etc/init.d/S30giftd /usr/sbin/giftd_wrapper
...
$(GIFT_BUILD_DIR)/.staged: $(GIFT_BUILD_DIR)/.built
	...
	sed -i -e 's|^prefix=$(TARGET_PREFIX)|prefix=$(STAGING_PREFIX)|' $(STAGING_LIB_DIR)/pkgconfig/libgift.pc
...
#
# Binaries should be installed into $(GIFT_IPK_DIR)$(TARGET_PREFIX)/sbin or $(GIFT_IPK_DIR)$(TARGET_PREFIX)/bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(GIFT_IPK_DIR)$(TARGET_PREFIX)/{lib,include}
# Configuration files should be installed in $(GIFT_IPK_DIR)$(TARGET_PREFIX)/etc/gift/...
# Documentation files should be installed in $(GIFT_IPK_DIR)$(TARGET_PREFIX)/doc/gift/...
# Daemon startup scripts should be installed in $(GIFT_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/S??gift
#
...
$(GIFT_IPK): $(GIFT_BUILD_DIR)/.built
	...
	$(INSTALL) -d $(GIFT_IPK_DIR)$(TARGET_PREFIX)/etc/init.d
	$(INSTALL) -m 755 $(GIFT_SOURCE_DIR)/S30giftd $(GIFT_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/S30giftd
	$(INSTALL) -d $(GIFT_IPK_DIR)$(TARGET_PREFIX)/sbin
	$(INSTALL) -m 755 $(GIFT_SOURCE_DIR)/giftd_wrapper $(GIFT_IPK_DIR)$(TARGET_PREFIX)/sbin
	$(INSTALL) -d $(GIFT_IPK_DIR)/CONTROL
