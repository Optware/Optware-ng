...
PERL-DIGEST-SHA1_SITE=http://$(PERL_CPAN_SITE)/CPAN/authors/id/G/GA/GAAS
...
PERL-DIGEST-SHA1_IPK_VERSION=5
...
$(DL_DIR)/$(PERL-DIGEST-SHA1_SOURCE):
	$(WGET) -P $(@D) $(PERL-DIGEST-SHA1_SITE)/$(@F) || \
...
$(PERL-DIGEST-SHA1_IPK): $(PERL-DIGEST-SHA1_BUILD_DIR)/.built
	...
	find $(PERL-DIGEST-SHA1_IPK_DIR)$(TARGET_PREFIX) -name 'perllocal.pod' -exec rm -f {} \;
	(cd $(PERL-DIGEST-SHA1_IPK_DIR)$(TARGET_PREFIX)/lib/perl5 ; \
		find . -name '*.so' -exec chmod +w {} \; ; \
		find . -name '*.so' -exec $(STRIP_COMMAND) {} \; ; \
		find . -name '*.so' -exec chmod -w {} \; ; \
	)
	find $(PERL-DIGEST-SHA1_IPK_DIR)$(TARGET_PREFIX) -type d -exec chmod go+rx {} \;
