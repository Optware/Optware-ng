...
# HELLO_CONFFILES=$(TARGET_PREFIX)/etc/hello.conf $(TARGET_PREFIX)/etc/init.d/SXXhello
...
#
# Binaries should be installed into $(HELLO_IPK_DIR)$(TARGET_PREFIX)/sbin or $(HELLO_IPK_DIR)$(TARGET_PREFIX)/bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(HELLO_IPK_DIR)$(TARGET_PREFIX)/{lib,include}
# Configuration files should be installed in $(HELLO_IPK_DIR)$(TARGET_PREFIX)/etc/hello/...
# Documentation files should be installed in $(HELLO_IPK_DIR)$(TARGET_PREFIX)/doc/hello/...
# Daemon startup scripts should be installed in $(HELLO_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/S??hello
#
...
$(HELLO_IPK): $(HELLO_BUILD_DIR)/.built
	...
	rm -f $(HELLO_IPK_DIR)$(TARGET_PREFIX)/share/info/dir
#	$(INSTALL) -d $(HELLO_IPK_DIR)$(TARGET_PREFIX)/etc/
#	$(INSTALL) -m 644 $(HELLO_SOURCE_DIR)/hello.conf $(HELLO_IPK_DIR)$(TARGET_PREFIX)/etc/hello.conf
#	$(INSTALL) -d $(HELLO_IPK_DIR)$(TARGET_PREFIX)/etc/init.d
#	$(INSTALL) -m 755 $(HELLO_SOURCE_DIR)/rc.hello $(HELLO_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/SXXhello
#	sed -i -e '/^#!/aOPTWARE_TARGET=${OPTWARE_TARGET}' $(HELLO_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/SXXhello
