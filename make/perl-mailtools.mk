...
PERL-MAILTOOLS_SITE=http://$(PERL_CPAN_SITE)/CPAN/authors/id/M/MA/MARKOV
...
$(DL_DIR)/$(PERL-MAILTOOLS_SOURCE):
	$(WGET) -P $(@D) $(PERL-MAILTOOLS_SITE)/$(@F) || \
	$(WGET) -P $(@D) $(FREEBSD_DISTFILES)/$(@F) || \
	$(WGET) -P $(@D) $(SOURCES_NLO_SITE)/$(@F)

perl-mailtools-source: $(DL_DIR)/$(PERL-MAILTOOLS_SOURCE) $(PERL-MAILTOOLS_PATCHES)

$(PERL-MAILTOOLS_BUILD_DIR)/.configured: $(DL_DIR)/$(PERL-MAILTOOLS_SOURCE) $(PERL-MAILTOOLS_PATCHES) make/perl-mailtools.mk
...
$(PERL-MAILTOOLS_IPK): $(PERL-MAILTOOLS_BUILD_DIR)/.built
	...
	find $(PERL-MAILTOOLS_IPK_DIR)$(TARGET_PREFIX) -name 'perllocal.pod' -exec rm -f {} \;
	(cd $(PERL-MAILTOOLS_IPK_DIR)$(TARGET_PREFIX)/lib/perl5 ; \
		find . -name '*.so' -exec chmod +w {} \; ; \
		find . -name '*.so' -exec $(STRIP_COMMAND) {} \; ; \
		find . -name '*.so' -exec chmod -w {} \; ; \
	)
	find $(PERL-MAILTOOLS_IPK_DIR)$(TARGET_PREFIX) -type d -exec chmod go+rx {} \;
