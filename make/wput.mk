...
#WPUT_CONFFILES=$(TARGET_PREFIX)/etc/wput.conf $(TARGET_PREFIX)/etc/init.d/SXXwput
...
#
# Binaries should be installed into $(WPUT_IPK_DIR)$(TARGET_PREFIX)/sbin or $(WPUT_IPK_DIR)$(TARGET_PREFIX)/bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(WPUT_IPK_DIR)$(TARGET_PREFIX)/{lib,include}
# Configuration files should be installed in $(WPUT_IPK_DIR)$(TARGET_PREFIX)/etc/wput/...
# Documentation files should be installed in $(WPUT_IPK_DIR)$(TARGET_PREFIX)/doc/wput/...
# Daemon startup scripts should be installed in $(WPUT_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/S??wput
#
# You may need to patch your application to make it use these locations.
#
$(WPUT_IPK): $(WPUT_BUILD_DIR)/.built
	rm -rf $(WPUT_IPK_DIR) $(BUILD_DIR)/wput_*_$(TARGET_ARCH).ipk
	$(INSTALL) -d $(WPUT_IPK_DIR)$(TARGET_PREFIX)/bin/
	$(INSTALL) -d $(WPUT_IPK_DIR)$(TARGET_PREFIX)/share/man/man1
		CPPFLAGS="$(STAGING_CPPFLAGS) $(WPUT_CPPFLAGS)" \
		LDFLAGS="$(STAGING_LDFLAGS) $(WPUT_LDFLAGS)" \
	$(MAKE) -C $(WPUT_BUILD_DIR) prefix=$(WPUT_IPK_DIR)$(TARGET_PREFIX) install
	$(STRIP_COMMAND) $(WPUT_IPK_DIR)$(TARGET_PREFIX)/bin/*
#	$(INSTALL) -d $(WPUT_IPK_DIR)$(TARGET_PREFIX)/etc/
#	$(INSTALL) -m 644 $(WPUT_SOURCE_DIR)/wput.conf $(WPUT_IPK_DIR)$(TARGET_PREFIX)/etc/wput.conf
#	$(INSTALL) -d $(WPUT_IPK_DIR)$(TARGET_PREFIX)/etc/init.d
#	$(INSTALL) -m 755 $(WPUT_SOURCE_DIR)/rc.wput $(WPUT_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/SXXwput
#	sed -i -e '/^#!/aOPTWARE_TARGET=${OPTWARE_TARGET}' $(XINETD_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/SXXwput
