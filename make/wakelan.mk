...
#
# Binaries should be installed into $(WAKELAN_IPK_DIR)$(TARGET_PREFIX)/sbin or $(WAKELAN_IPK_DIR)$(TARGET_PREFIX)/bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(WAKELAN_IPK_DIR)$(TARGET_PREFIX)/{lib,include}
# Configuration files should be installed in $(WAKELAN_IPK_DIR)$(TARGET_PREFIX)/etc/wakelan/...
# Documentation files should be installed in $(WAKELAN_IPK_DIR)$(TARGET_PREFIX)/doc/wakelan/...
# Daemon startup scripts should be installed in $(WAKELAN_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/S??wakelan
#
# You may need to patch your application to make it use these locations.
#
$(WAKELAN_IPK): $(WAKELAN_BUILD_DIR)/.built
	rm -rf $(WAKELAN_IPK_DIR) $(BUILD_DIR)/wakelan_*_$(TARGET_ARCH).ipk
	$(INSTALL) -d $(WAKELAN_IPK_DIR)$(TARGET_PREFIX)/bin
	$(INSTALL) -d $(WAKELAN_IPK_DIR)$(TARGET_PREFIX)/man/man1
