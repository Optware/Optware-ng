...
#DUMP_CONFFILES=$(TARGET_PREFIX)/etc/dump.conf $(TARGET_PREFIX)/etc/init.d/SXXdump
...
#
# Binaries should be installed into $(DUMP_IPK_DIR)$(TARGET_PREFIX)/sbin or $(DUMP_IPK_DIR)$(TARGET_PREFIX)/bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(DUMP_IPK_DIR)$(TARGET_PREFIX)/{lib,include}
# Configuration files should be installed in $(DUMP_IPK_DIR)$(TARGET_PREFIX)/etc/dump/...
# Documentation files should be installed in $(DUMP_IPK_DIR)$(TARGET_PREFIX)/doc/dump/...
# Daemon startup scripts should be installed in $(DUMP_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/S??dump
#
...
$(DUMP_IPK): $(DUMP_BUILD_DIR)/.built
	...
	$(MAKE) -C $(DUMP_BUILD_DIR) install \
	    prefix=$(DUMP_IPK_DIR)$(TARGET_PREFIX) \
	    INSTALLBIN='/usr/bin/install -m 0755' \
	    INSTALLMAN='/usr/bin/install -m 0644'
	$(STRIP_COMMAND) $(DUMP_IPK_DIR)$(TARGET_PREFIX)/sbin/dump $(DUMP_IPK_DIR)$(TARGET_PREFIX)/sbin/restore $(DUMP_IPK_DIR)$(TARGET_PREFIX)/sbin/rmt
