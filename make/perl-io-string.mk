...
$(PERL-IO-STRING_BUILD_DIR)/.configured: $(DL_DIR)/$(PERL-IO-STRING_SOURCE) $(PERL-IO-STRING_PATCHES) make/perl-io-string.mk
	$(MAKE) perl-stage
...
$(PERL-IO-STRING_IPK): $(PERL-IO-STRING_BUILD_DIR)/.built
	...
	find $(PERL-IO-STRING_IPK_DIR)$(TARGET_PREFIX) -name 'perllocal.pod' -exec rm -f {} \;
	(cd $(PERL-IO-STRING_IPK_DIR)$(TARGET_PREFIX)/lib/perl5 ; \
		find . -name '*.so' -exec chmod +w {} \; ; \
		find . -name '*.so' -exec $(STRIP_COMMAND) {} \; ; \
		find . -name '*.so' -exec chmod -w {} \; ; \
	)
	find $(PERL-IO-STRING_IPK_DIR)$(TARGET_PREFIX) -type d -exec chmod go+rx {} \;
