...
#
PY-CHARACTERISTIC_IPK_VERSION=1

#
# PY-CHARACTERISTIC_CONFFILES should be a list of user-editable files
#PY-CHARACTERISTIC_CONFFILES=/opt/etc/py-characteristic.conf /opt/etc/init.d/SXXpy-characteristic
...
#
# Binaries should be installed into $(PY-CHARACTERISTIC_IPK_DIR)/opt/sbin or $(PY-CHARACTERISTIC_IPK_DIR)/opt/bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(PY-CHARACTERISTIC_IPK_DIR)/opt/{lib,include}
# Configuration files should be installed in $(PY-CHARACTERISTIC_IPK_DIR)/opt/etc/py-characteristic/...
# Documentation files should be installed in $(PY-CHARACTERISTIC_IPK_DIR)/opt/doc/py-characteristic/...
# Daemon startup scripts should be installed in $(PY-CHARACTERISTIC_IPK_DIR)/opt/etc/init.d/S??py-characteristic
#
...
$(PY26-CHARACTERISTIC_IPK): $(PY-CHARACTERISTIC_BUILD_DIR)/.built
	...
	$(HOST_STAGING_PREFIX)/bin/python2.6 setup.py install --root=$(PY26-CHARACTERISTIC_IPK_DIR) --prefix=/opt)
#	rm -f $(PY26-CHARACTERISTIC_IPK_DIR)/opt/bin/easy_install
...
$(PY27-CHARACTERISTIC_IPK): $(PY-CHARACTERISTIC_BUILD_DIR)/.built
	...
	$(HOST_STAGING_PREFIX)/bin/python2.7 setup.py install --root=$(PY27-CHARACTERISTIC_IPK_DIR) --prefix=/opt)
	rm -f $(PY27-CHARACTERISTIC_IPK_DIR)/opt/bin/easy_install
...
$(PY3-CHARACTERISTIC_IPK): $(PY-CHARACTERISTIC_BUILD_DIR)/.built
	...
	$(HOST_STAGING_PREFIX)/bin/python$(PYTHON3_VERSION_MAJOR) setup.py install --root=$(PY3-CHARACTERISTIC_IPK_DIR) --prefix=/opt)
	rm -f $(PY3-CHARACTERISTIC_IPK_DIR)/opt/bin/easy_install
