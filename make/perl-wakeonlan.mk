...
$(DL_DIR)/$(PERL-WAKEONLAN_SOURCE):
	$(WGET) -P $(@D) $(PERL-WAKEONLAN_SITE)/$(@F) || \
	$(WGET) -P $(@D) $(FREEBSD_DISTFILES)/$(@F) || \
	$(WGET) -P $(@D) $(SOURCES_NLO_SITE)/$(@F)

perl-wakeonlan-source: $(DL_DIR)/$(PERL-WAKEONLAN_SOURCE) $(PERL-WAKEONLAN_PATCHES)

$(PERL-WAKEONLAN_BUILD_DIR)/.configured: $(DL_DIR)/$(PERL-WAKEONLAN_SOURCE) $(PERL-WAKEONLAN_PATCHES) make/perl-wakeonlan.mk
	make perl-stage
...
$(PERL-WAKEONLAN_IPK): $(PERL-WAKEONLAN_BUILD_DIR)/.built
	...
	find $(PERL-WAKEONLAN_IPK_DIR)$(TARGET_PREFIX) -name 'perllocal.pod' -exec rm -f {} \;
	(cd $(PERL-WAKEONLAN_IPK_DIR)$(TARGET_PREFIX)/lib/perl5 ; \
		find . -name '*.so' -exec chmod +w {} \; ; \
		find . -name '*.so' -exec $(STRIP_COMMAND) {} \; ; \
		find . -name '*.so' -exec chmod -w {} \; ; \
	)
	find $(PERL-WAKEONLAN_IPK_DIR)$(TARGET_PREFIX) -type d -exec chmod go+rx {} \;
