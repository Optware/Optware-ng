...
$(IMAP_BUILD_DIR)/.built: $(IMAP_BUILD_DIR)/.configured
	...
	$(MAKE) -C $(IMAP_BUILD_DIR) slx CC=$(TARGET_CC) AR=$(TARGET_AR) RANLIB=$(TARGET_RANLIB) EXTRACFLAGS="$(STAGING_CPPFLAGS) $(IMAP_CPPFLAGS)" EXTRALDFLAGS="$(STAGING_LDFLAGS) $(IMAP_LDFLAGS)" SSLDIR=$(TARGET_PREFIX) SSLINCLUDE=$(STAGING_INCLUDE_DIR)/openssl SHLIBBASE=c-client SHLIBNAME=libc-client.so.0
