...
$(PERL-DATE-CALC_BUILD_DIR)/.configured: $(DL_DIR)/$(PERL-DATE-CALC_SOURCE) $(PERL-DATE-CALC_PATCHES) make/perl-date-calc.mk
	$(MAKE) perl-stage
...
$(PERL-DATE-CALC_IPK): $(PERL-DATE-CALC_BUILD_DIR)/.built
	...
	find $(PERL-DATE-CALC_IPK_DIR)$(TARGET_PREFIX) -name 'perllocal.pod' -exec rm -f {} \;
	(cd $(PERL-DATE-CALC_IPK_DIR)$(TARGET_PREFIX)/lib/perl5 ; \
		find . -name '*.so' -exec chmod +w {} \; ; \
		find . -name '*.so' -exec $(STRIP_COMMAND) {} \; ; \
		find . -name '*.so' -exec chmod -w {} \; ; \
	)
	find $(PERL-DATE-CALC_IPK_DIR)$(TARGET_PREFIX) -type d -exec chmod go+rx {} \;
