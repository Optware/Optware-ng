...
# You must replace "groff" and "GROFF" with the lower case name and
...
#
# GROFF_CONFFILES should be a list of user-editable files
#GROFF_CONFFILES=
...
#
$(STAGING_DIR)/opt/lib/libgroff.so.$(GROFF_VERSION): $(GROFF_BUILD_DIR)/.built
	install -d $(STAGING_DIR)/opt/include
	install -m 644 $(GROFF_BUILD_DIR)/groff.h $(STAGING_DIR)/opt/include
	install -d $(STAGING_DIR)/opt/lib
	install -m 644 $(GROFF_BUILD_DIR)/libgroff.a $(STAGING_DIR)/opt/lib
	install -m 644 $(GROFF_BUILD_DIR)/libgroff.so.$(GROFF_VERSION) $(STAGING_DIR)/opt/lib
	cd $(STAGING_DIR)/opt/lib && ln -fs libgroff.so.$(GROFF_VERSION) libgroff.so.1
	cd $(STAGING_DIR)/opt/lib && ln -fs libgroff.so.$(GROFF_VERSION) libgroff.so

groff-stage: $(STAGING_DIR)/opt/lib/libgroff.so.$(GROFF_VERSION)
...
#	$(STRIP_COMMAND) $(GROFF_BUILD_DIR)/groff -o $(GROFF_IPK_DIR)/opt/bin/groff
#	install -d $(GROFF_IPK_DIR)/opt/etc/
#	install -m 755 $(GROFF_SOURCE_DIR)/groff.conf $(GROFF_IPK_DIR)/opt/etc/groff.conf
#	install -d $(GROFF_IPK_DIR)/opt/etc/init.d
#	install -m 755 $(GROFF_SOURCE_DIR)/rc.groff $(GROFF_IPK_DIR)/opt/etc/init.d/SXXgroff
	install -d $(GROFF_IPK_DIR)/CONTROL
	install -m 644 $(GROFF_SOURCE_DIR)/control $(GROFF_IPK_DIR)/CONTROL/control
#	install -m 644 $(GROFF_SOURCE_DIR)/postinst $(GROFF_IPK_DIR)/CONTROL/postinst
#	install -m 644 $(GROFF_SOURCE_DIR)/prerm $(GROFF_IPK_DIR)/CONTROL/prerm
	echo $(GROFF_CONFFILES) | sed -e 's/ /\n/g' > $(GROFF_IPK_DIR)/CONTROL/conffiles
