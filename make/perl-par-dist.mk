...
PERL-PAR-DIST_SITE=http://$(PERL_CPAN_SITE)/CPAN/authors/id/S/SM/SMUELLER
...
$(DL_DIR)/$(PERL-PAR-DIST_SOURCE):
	$(WGET) -P $(@D) $(PERL-PAR-DIST_SITE)/$(@F) || \
	$(WGET) -P $(@D) $(FREEBSD_DISTFILES)/$(@F) || \
	$(WGET) -P $(@D) $(SOURCES_NLO_SITE)/$(@F)

perl-par-dist-source: $(DL_DIR)/$(PERL-PAR-DIST_SOURCE) $(PERL-PAR-DIST_PATCHES)

$(PERL-PAR-DIST_BUILD_DIR)/.configured: $(DL_DIR)/$(PERL-PAR-DIST_SOURCE) $(PERL-PAR-DIST_PATCHES) make/perl-par-dist.mk
	$(MAKE) perl-stage
...
$(PERL-PAR-DIST_IPK): $(PERL-PAR-DIST_BUILD_DIR)/.built
	...
	find $(PERL-PAR-DIST_IPK_DIR)$(TARGET_PREFIX) -name 'perllocal.pod' -exec rm -f {} \;
	(cd $(PERL-PAR-DIST_IPK_DIR)$(TARGET_PREFIX)/lib/perl5 ; \
		find . -name '*.so' -exec chmod +w {} \; ; \
		find . -name '*.so' -exec $(STRIP_COMMAND) {} \; ; \
		find . -name '*.so' -exec chmod -w {} \; ; \
	)
	find $(PERL-PAR-DIST_IPK_DIR)$(TARGET_PREFIX) -type d -exec chmod go+rx {} \;
