...
PERL-CLASS-ACCESSOR_SITE=http://$(PERL_CPAN_SITE)/CPAN/authors/id/K/KA/KASEI
PERL-CLASS-ACCESSOR_VERSION=0.34
...
PERL-CLASS-ACCESSOR_IPK_VERSION=4
...
$(DL_DIR)/$(PERL-CLASS-ACCESSOR_SOURCE):
	$(WGET) -P $(@D) $(PERL-CLASS-ACCESSOR_SITE)/$(@F) || \
	$(WGET) -P $(@D) $(FREEBSD_DISTFILES)/$(@F) || \
	$(WGET) -P $(@D) $(SOURCES_NLO_SITE)/$(@F)

perl-class-accessor-source: $(DL_DIR)/$(PERL-CLASS-ACCESSOR_SOURCE) $(PERL-CLASS-ACCESSOR_PATCHES)

$(PERL-CLASS-ACCESSOR_BUILD_DIR)/.configured: $(DL_DIR)/$(PERL-CLASS-ACCESSOR_SOURCE) $(PERL-CLASS-ACCESSOR_PATCHES) make/perl-class-accessor.mk
	$(MAKE) perl-stage
...
$(PERL-CLASS-ACCESSOR_IPK): $(PERL-CLASS-ACCESSOR_BUILD_DIR)/.built
	...
	find $(PERL-CLASS-ACCESSOR_IPK_DIR)$(TARGET_PREFIX) -name 'perllocal.pod' -exec rm -f {} \;
	(cd $(PERL-CLASS-ACCESSOR_IPK_DIR)$(TARGET_PREFIX)/lib/perl5 ; \
		find . -name '*.so' -exec chmod +w {} \; ; \
		find . -name '*.so' -exec $(STRIP_COMMAND) {} \; ; \
		find . -name '*.so' -exec chmod -w {} \; ; \
	)
	find $(PERL-CLASS-ACCESSOR_IPK_DIR)$(TARGET_PREFIX) -type d -exec chmod go+rx {} \;
