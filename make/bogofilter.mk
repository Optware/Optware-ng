...
BOGOFILTER_CONFFILES=$(TARGET_PREFIX)/etc/bogofilter.conf
...
$(BOGOFILTER_IPK): $(BOGOFILTER_BUILD_DIR)/.built
	...
	$(INSTALL) -d $(BOGOFILTER_IPK_DIR)$(TARGET_PREFIX)/bin/
	$(INSTALL) -d $(BOGOFILTER_IPK_DIR)$(TARGET_PREFIX)/sbin/
	$(INSTALL) -d $(BOGOFILTER_IPK_DIR)$(TARGET_PREFIX)/doc/bogofilter/
	$(INSTALL) -d $(BOGOFILTER_IPK_DIR)$(TARGET_PREFIX)/etc/
	$(INSTALL) -d $(BOGOFILTER_IPK_DIR)$(TARGET_PREFIX)/man/man1/
	$(INSTALL) -d $(BOGOFILTER_IPK_DIR)$(TARGET_PREFIX)/var/spool/bogofilter
	$(MAKE) -C $(BOGOFILTER_BUILD_DIR) DESTDIR=$(BOGOFILTER_IPK_DIR) install
	$(STRIP_COMMAND) $(BOGOFILTER_IPK_DIR)$(TARGET_PREFIX)/bin/bogofilter
	$(STRIP_COMMAND) $(BOGOFILTER_IPK_DIR)$(TARGET_PREFIX)/bin/bogolexer
	$(STRIP_COMMAND) $(BOGOFILTER_IPK_DIR)$(TARGET_PREFIX)/bin/bogotune
	$(STRIP_COMMAND) $(BOGOFILTER_IPK_DIR)$(TARGET_PREFIX)/bin/bogoutil
	for i in "README.db" "README.sqlite" "README.tdb" "README.validation" \
	    ...
	    $(INSTALL) -m 644 $(BOGOFILTER_BUILD_DIR)/doc/$$i $(BOGOFILTER_IPK_DIR)$(TARGET_PREFIX)/doc/bogofilter/$$i ; \
	done
	$(INSTALL) -m 644 $(BOGOFILTER_SOURCE_DIR)/master.cf.patch $(BOGOFILTER_IPK_DIR)$(TARGET_PREFIX)/doc/bogofilter/master.cf.patch
	mv $(BOGOFILTER_IPK_DIR)$(TARGET_PREFIX)/etc/bogofilter.cf.example $(BOGOFILTER_IPK_DIR)$(TARGET_PREFIX)/doc/bogofilter/bogofilter.cf.example
	$(INSTALL) -m 644 $(BOGOFILTER_SOURCE_DIR)/bogofilter.conf $(BOGOFILTER_IPK_DIR)$(TARGET_PREFIX)/etc/bogofilter.conf
	$(INSTALL) -m 755 $(BOGOFILTER_SOURCE_DIR)/postfix-bogofilter.sh $(BOGOFILTER_IPK_DIR)$(TARGET_PREFIX)/sbin/postfix-bogofilter.sh
