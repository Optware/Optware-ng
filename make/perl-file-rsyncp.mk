...
PERL-FILE-RSYNCP_SITE=http://$(PERL_CPAN_SITE)/CPAN/authors/id/C/CB/CBARRATT
...
PERL-FILE-RSYNCP_IPK_VERSION=3
...
$(DL_DIR)/$(PERL-FILE-RSYNCP_SOURCE):
	$(WGET) -P $(@D) $(PERL-FILE-RSYNCP_SITE)/$(@F) || \
	$(WGET) -P $(@D) $(FREEBSD_DISTFILES)/$(@F) || \
	$(WGET) -P $(@D) $(SOURCES_NLO_SITE)/$(@F)

perl-file-rsyncp-source: $(DL_DIR)/$(PERL-FILE-RSYNCP_SOURCE) $(PERL-FILE-RSYNCP_PATCHES)

$(PERL-FILE-RSYNCP_BUILD_DIR)/.configured: $(DL_DIR)/$(PERL-FILE-RSYNCP_SOURCE) $(PERL-FILE-RSYNCP_PATCHES) make/perl-file-rsyncp.mk
...
$(PERL-FILE-RSYNCP_IPK): $(PERL-FILE-RSYNCP_BUILD_DIR)/.built
	...
	find $(PERL-FILE-RSYNCP_IPK_DIR)$(TARGET_PREFIX) -name 'perllocal.pod' -exec rm -f {} \;
	(cd $(PERL-FILE-RSYNCP_IPK_DIR)$(TARGET_PREFIX)/lib/perl5 ; \
		find . -name '*.so' -exec chmod +w {} \; ; \
		find . -name '*.so' -exec $(STRIP_COMMAND) {} \; ; \
		find . -name '*.so' -exec chmod -w {} \; ; \
	)
	find $(PERL-FILE-RSYNCP_IPK_DIR)$(TARGET_PREFIX) -type d -exec chmod go+rx {} \;
