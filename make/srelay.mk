...
SRELAY_CONFFILES=$(TARGET_PREFIX)/etc/srelay.conf
...
#
# Binaries should be installed into $(SRELAY_IPK_DIR)$(TARGET_PREFIX)/sbin or $(SRELAY_IPK_DIR)$(TARGET_PREFIX)/bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(SRELAY_IPK_DIR)$(TARGET_PREFIX)/{lib,include}
# Configuration files should be installed in $(SRELAY_IPK_DIR)$(TARGET_PREFIX)/etc/srelay/...
# Documentation files should be installed in $(SRELAY_IPK_DIR)$(TARGET_PREFIX)/doc/srelay/...
# Daemon startup scripts should be installed in $(SRELAY_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/S??srelay
#
...
#	$(MAKE) -C $(SRELAY_BUILD_DIR) DESTDIR=$(SRELAY_IPK_DIR) install
	$(INSTALL) -d $(SRELAY_IPK_DIR)$(TARGET_PREFIX)/etc/ $(SRELAY_IPK_DIR)$(TARGET_PREFIX)/bin/ $(SRELAY_IPK_DIR)$(TARGET_PREFIX)/share/man/man8/
	$(INSTALL) -m 644 $(SRELAY_BUILD_DIR)/srelay.conf $(SRELAY_IPK_DIR)$(TARGET_PREFIX)/etc/
	$(INSTALL) -m 755 $(SRELAY_BUILD_DIR)/srelay $(SRELAY_IPK_DIR)$(TARGET_PREFIX)/bin/
	$(STRIP_COMMAND) $(SRELAY_IPK_DIR)$(TARGET_PREFIX)/bin/srelay
	$(INSTALL) -m 644 $(SRELAY_BUILD_DIR)/srelay.8 $(SRELAY_IPK_DIR)$(TARGET_PREFIX)/share/man/man8/
#	$(INSTALL) -d $(SRELAY_IPK_DIR)$(TARGET_PREFIX)/etc/init.d
#	$(INSTALL) -m 755 $(SRELAY_SOURCE_DIR)/rc.srelay $(SRELAY_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/SXXsrelay
#	sed -i -e '/^#!/aOPTWARE_TARGET=${OPTWARE_TARGET}' $(XINETD_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/SXXsrelay
