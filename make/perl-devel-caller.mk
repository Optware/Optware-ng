...
PERL-DEVEL-CALLER_IPK_VERSION=3
...
$(PERL-DEVEL-CALLER_BUILD_DIR)/.built: $(PERL-DEVEL-CALLER_BUILD_DIR)/.configured
	...
	$(MAKE) -C $(@D) \
		$(TARGET_CONFIGURE_OPTS) \
		LD=$(TARGET_CC) \
		CCFLAGS="$(STAGING_CPPFLAGS) $(PERL_MODULES_CFLAGS) $(PERL-DEVEL-CALLER_CPPFLAGS)" \
		LDFLAGS="$(STAGING_LDFLAGS) $(PERL_MODULES_LDFLAGS) $(PERL-DEVEL-CALLER_LDFLAGS)" \
		LDDLFLAGS="-shared $(STAGING_LDFLAGS) $(PERL_MODULES_LDFLAGS) $(PERL-DEVEL-CALLER_LDFLAGS)" \
		PERL5LIB="$(STAGING_LIB_DIR)/perl5/site_perl"
	touch $@
