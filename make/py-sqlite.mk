...
#PY-SQLITE_CONFFILES=$(TARGET_PREFIX)/etc/py-sqlite.conf $(TARGET_PREFIX)/etc/init.d/SXXpy-sqlite
...
#
# Binaries should be installed into $(PY-SQLITE_IPK_DIR)$(TARGET_PREFIX)/sbin or $(PY-SQLITE_IPK_DIR)$(TARGET_PREFIX)/bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(PY-SQLITE_IPK_DIR)$(TARGET_PREFIX)/{lib,include}
# Configuration files should be installed in $(PY-SQLITE_IPK_DIR)$(TARGET_PREFIX)/etc/py-sqlite/...
# Documentation files should be installed in $(PY-SQLITE_IPK_DIR)$(TARGET_PREFIX)/doc/py-sqlite/...
# Daemon startup scripts should be installed in $(PY-SQLITE_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/S??py-sqlite
#
...
$(PY-SQLITE_IPK): $(PY-SQLITE_BUILD_DIR)/.built
	...
	PYTHONPATH=$(STAGING_LIB_DIR)/python2.4/site-packages \
	    ...
	    --root=$(PY-SQLITE_IPK_DIR) --prefix=$(TARGET_PREFIX); \
	)
	$(STRIP_COMMAND) $(PY-SQLITE_IPK_DIR)$(TARGET_PREFIX)/lib/python2.4/site-packages/pysqlite2/_sqlite.so
