...
#OSSP_JS_CONFFILES=$(TARGET_PREFIX)/etc/ossp-js.conf $(TARGET_PREFIX)/etc/init.d/SXXossp-js
...
#
# Binaries should be installed into $(OSSP_JS_IPK_DIR)$(TARGET_PREFIX)/sbin or $(OSSP_JS_IPK_DIR)$(TARGET_PREFIX)/bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(OSSP_JS_IPK_DIR)$(TARGET_PREFIX)/{lib,include}
# Configuration files should be installed in $(OSSP_JS_IPK_DIR)$(TARGET_PREFIX)/etc/ossp-js/...
# Documentation files should be installed in $(OSSP_JS_IPK_DIR)$(TARGET_PREFIX)/doc/ossp-js/...
# Daemon startup scripts should be installed in $(OSSP_JS_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/S??ossp-js
#
...
$(OSSP_JS_IPK): $(OSSP_JS_BUILD_DIR)/.built
	...
	$(STRIP_COMMAND) $(OSSP_JS_IPK_DIR)$(TARGET_PREFIX)/bin/js $(OSSP_JS_IPK_DIR)$(TARGET_PREFIX)/lib/libjs.so.*
#	$(INSTALL) -d $(OSSP_JS_IPK_DIR)$(TARGET_PREFIX)/etc/
#	$(INSTALL) -m 644 $(OSSP_JS_SOURCE_DIR)/ossp-js.conf $(OSSP_JS_IPK_DIR)$(TARGET_PREFIX)/etc/ossp-js.conf
#	$(INSTALL) -d $(OSSP_JS_IPK_DIR)$(TARGET_PREFIX)/etc/init.d
#	$(INSTALL) -m 755 $(OSSP_JS_SOURCE_DIR)/rc.ossp-js $(OSSP_JS_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/SXXossp-js
#	sed -i -e '/^#!/aOPTWARE_TARGET=${OPTWARE_TARGET}' $(XINETD_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/SXXossp-js
