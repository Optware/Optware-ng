...
#
# Binaries should be installed into $(FREEZE_IPK_DIR)$(TARGET_PREFIX)/sbin or $(FREEZE_IPK_DIR)$(TARGET_PREFIX)/bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(FREEZE_IPK_DIR)$(TARGET_PREFIX)/{lib,include}
# Configuration files should be installed in $(FREEZE_IPK_DIR)$(TARGET_PREFIX)/etc/freeze/...
# Documentation files should be installed in $(FREEZE_IPK_DIR)$(TARGET_PREFIX)/doc/freeze/...
# Daemon startup scripts should be installed in $(FREEZE_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/S??freeze
#
# You may need to patch your application to make it use these locations.
#
$(FREEZE_IPK): $(FREEZE_BUILD_DIR)/.built
	rm -rf $(FREEZE_IPK_DIR) $(BUILD_DIR)/freeze_*_$(TARGET_ARCH).ipk
	$(INSTALL) -d $(FREEZE_IPK_DIR)$(TARGET_PREFIX)/bin
	$(INSTALL) -d $(FREEZE_IPK_DIR)$(TARGET_PREFIX)/share/man/man1
	$(MAKE) -C $(FREEZE_BUILD_DIR) prefix=$(FREEZE_IPK_DIR)$(TARGET_PREFIX) install
	$(STRIP_COMMAND) $(FREEZE_IPK_DIR)$(TARGET_PREFIX)/bin/freeze
	$(STRIP_COMMAND) $(FREEZE_IPK_DIR)$(TARGET_PREFIX)/bin/statist
