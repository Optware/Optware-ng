...
PERL-BUSINESS-ISBN_SITE=http://$(PERL_CPAN_SITE)/CPAN/authors/id/B/BD/BDFOY
PERL-BUSINESS-ISBN_VERSION=2.09
...
PERL-BUSINESS-ISBN_IPK_VERSION=3
...
$(DL_DIR)/$(PERL-BUSINESS-ISBN_SOURCE):
	$(WGET) -P $(@D) $(PERL-BUSINESS-ISBN_SITE)/$(@F) || \
	$(WGET) -P $(@D) $(FREEBSD_DISTFILES)/$(@F) || \
	$(WGET) -P $(@D) $(SOURCES_NLO_SITE)/$(@F)

perl-business-isbn-source: $(DL_DIR)/$(PERL-BUSINESS-ISBN_SOURCE) $(PERL-BUSINESS-ISBN_PATCHES)

$(PERL-BUSINESS-ISBN_BUILD_DIR)/.configured: $(DL_DIR)/$(PERL-BUSINESS-ISBN_SOURCE) $(PERL-BUSINESS-ISBN_PATCHES) make/perl-business-isbn.mk
...
$(PERL-BUSINESS-ISBN_IPK): $(PERL-BUSINESS-ISBN_BUILD_DIR)/.built
	...
	find $(PERL-BUSINESS-ISBN_IPK_DIR)$(TARGET_PREFIX) -name 'perllocal.pod' -exec rm -f {} \;
	(cd $(PERL-BUSINESS-ISBN_IPK_DIR)$(TARGET_PREFIX)/lib/perl5 ; \
		find . -name '*.so' -exec chmod +w {} \; ; \
		find . -name '*.so' -exec $(STRIP_COMMAND) {} \; ; \
		find . -name '*.so' -exec chmod -w {} \; ; \
	)
	find $(PERL-BUSINESS-ISBN_IPK_DIR)$(TARGET_PREFIX) -type d -exec chmod go+rx {} \;
