...
#GED_CONFFILES=$(TARGET_PREFIX)/etc/ged.conf $(TARGET_PREFIX)/etc/init.d/SXXged
...
#
# Binaries should be installed into $(GED_IPK_DIR)$(TARGET_PREFIX)/sbin or $(GED_IPK_DIR)$(TARGET_PREFIX)/bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(GED_IPK_DIR)$(TARGET_PREFIX)/{lib,include}
# Configuration files should be installed in $(GED_IPK_DIR)$(TARGET_PREFIX)/etc/ged/...
# Documentation files should be installed in $(GED_IPK_DIR)$(TARGET_PREFIX)/doc/ged/...
# Daemon startup scripts should be installed in $(GED_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/S??ged
#
# You may need to patch your application to make it use these locations.
#
$(GED_IPK): $(GED_BUILD_DIR)/.built
	rm -rf $(GED_IPK_DIR) $(BUILD_DIR)/ged_*_$(TARGET_ARCH).ipk
	$(INSTALL) -d $(GED_IPK_DIR)$(TARGET_PREFIX)/bin
	$(INSTALL) -m 755 $(GED_BUILD_DIR)/ged $(GED_IPK_DIR)$(TARGET_PREFIX)/bin
	$(STRIP_COMMAND) $(GED_IPK_DIR)$(TARGET_PREFIX)/bin/ged
#	$(INSTALL) -d $(GED_IPK_DIR)$(TARGET_PREFIX)/etc/
#	$(INSTALL) -m 644 $(GED_SOURCE_DIR)/ged.conf $(GED_IPK_DIR)$(TARGET_PREFIX)/etc/ged.conf
#	$(INSTALL) -d $(GED_IPK_DIR)$(TARGET_PREFIX)/etc/init.d
#	$(INSTALL) -m 755 $(GED_SOURCE_DIR)/rc.ged $(GED_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/SXXged
#	sed -i -e '/^#!/aOPTWARE_TARGET=${OPTWARE_TARGET}' $(GED_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/SXXged
