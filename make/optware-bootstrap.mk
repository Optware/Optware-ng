...
$(OPTWARE-BOOTSTRAP_XSH): $(OPTWARE-BOOTSTRAP_BUILD_DIR)/.built $(OPTWARE-BOOTSTRAP_IPKS_DONE)
	...
	$(INSTALL) -d -m 755 \
		$(OPTWARE-BOOTSTRAP_IPK_DIR)$(TARGET_PREFIX)/etc \
		$(OPTWARE-BOOTSTRAP_IPK_DIR)$(TARGET_PREFIX)/lib \
		$(OPTWARE-BOOTSTRAP_IPK_DIR)$(TARGET_PREFIX)/var
ifneq ($(OPTWARE-BOOTSTRAP_LIBS),)
	cp -rip $(OPTWARE-BOOTSTRAP_LIBS) $(OPTWARE-BOOTSTRAP_IPK_DIR)$(TARGET_PREFIX)/lib
endif
	$(INSTALL) -d $(OPTWARE-BOOTSTRAP_IPK_DIR)$(TARGET_PREFIX)/var/lib
	$(INSTALL) -d $(OPTWARE-BOOTSTRAP_IPK_DIR)/etc/init.d
	$(INSTALL) -d -m 1755 $(OPTWARE-BOOTSTRAP_IPK_DIR)$(TARGET_PREFIX)/tmp
	$(INSTALL) -m 755 $(IPKG-OPT_SOURCE_DIR)/rc.optware $(OPTWARE-BOOTSTRAP_IPK_DIR)$(TARGET_PREFIX)/etc/
ifneq (, $(filter fsg3 fsg3v4, $(OPTWARE_TARGET)))
	...
	$(INSTALL) -d $(OPTWARE-BOOTSTRAP_IPK_DIR)$(TARGET_PREFIX)/etc/init.d
	if [ -e $(OPTWARE-BOOTSTRAP_SOURCE_DIR)/$(OPTWARE_TARGET)/S00optware ] ; then \
		$(INSTALL) -m 755 $(OPTWARE-BOOTSTRAP_SOURCE_DIR)/$(OPTWARE_TARGET)/S00optware \
			$(OPTWARE-BOOTSTRAP_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/ ; \
	fi
