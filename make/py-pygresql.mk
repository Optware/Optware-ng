...
PY24-PYGRESQL_DEPENDS=python24, py24-mx-base, postgresql
PY25-PYGRESQL_DEPENDS=python25, py25-mx-base, postgresql
PY-PYGRESQL_CONFLICTS=

#
# PY-PYGRESQL_IPK_VERSION should be incremented when the ipk changes.
#
PY-PYGRESQL_IPK_VERSION=4

#
# PY-PYGRESQL_CONFFILES should be a list of user-editable files
#PY-PYGRESQL_CONFFILES=$(TARGET_PREFIX)/etc/py-pygresql.conf $(TARGET_PREFIX)/etc/init.d/SXXpy-pygresql
...
#
# Binaries should be installed into $(PY-PYGRESQL_IPK_DIR)$(TARGET_PREFIX)/sbin or $(PY-PYGRESQL_IPK_DIR)$(TARGET_PREFIX)/bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(PY-PYGRESQL_IPK_DIR)$(TARGET_PREFIX)/{lib,include}
# Configuration files should be installed in $(PY-PYGRESQL_IPK_DIR)$(TARGET_PREFIX)/etc/py-pygresql/...
# Documentation files should be installed in $(PY-PYGRESQL_IPK_DIR)$(TARGET_PREFIX)/doc/py-pygresql/...
# Daemon startup scripts should be installed in $(PY-PYGRESQL_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/S??py-pygresql
#
...
$(PY24-PYGRESQL_IPK): $(PY-PYGRESQL_BUILD_DIR)/.built
	...
	(cd $(PY-PYGRESQL_BUILD_DIR)/2.4; \
	    ...
	    --root=$(PY24-PYGRESQL_IPK_DIR) --prefix=$(TARGET_PREFIX); \
	)
	$(STRIP_COMMAND) `find $(PY24-PYGRESQL_IPK_DIR)$(TARGET_PREFIX)/lib -name '*.so'`
...
$(PY25-PYGRESQL_IPK): $(PY-PYGRESQL_BUILD_DIR)/.built
	...
	(cd $(PY-PYGRESQL_BUILD_DIR)/2.5; \
	    ...
	    --root=$(PY25-PYGRESQL_IPK_DIR) --prefix=$(TARGET_PREFIX); \
	)
	$(STRIP_COMMAND) `find $(PY25-PYGRESQL_IPK_DIR)$(TARGET_PREFIX)/lib -name '*.so'`
