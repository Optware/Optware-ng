...
PERL-MIME-BASE64_SITE=http://$(PERL_CPAN_SITE)/CPAN/authors/id/G/GA/GAAS
...
PERL-MIME-BASE64_IPK_VERSION=4
...
$(DL_DIR)/$(PERL-MIME-BASE64_SOURCE):
	$(WGET) -P $(@D) $(PERL-MIME-BASE64_SITE)/$(@F) || \
	$(WGET) -P $(@D) $(FREEBSD_DISTFILES)/$(@F) || \
	$(WGET) -P $(@D) $(SOURCES_NLO_SITE)/$(@F)

perl-mime-base64-source: $(DL_DIR)/$(PERL-MIME-BASE64_SOURCE) $(PERL-MIME-BASE64_PATCHES)

$(PERL-MIME-BASE64_BUILD_DIR)/.configured: $(DL_DIR)/$(PERL-MIME-BASE64_SOURCE) $(PERL-MIME-BASE64_PATCHES) make/perl-mime-base64.mk
	make perl-stage
...
$(PERL-MIME-BASE64_IPK): $(PERL-MIME-BASE64_BUILD_DIR)/.built
	...
	find $(PERL-MIME-BASE64_IPK_DIR)$(TARGET_PREFIX) -name 'perllocal.pod' -exec rm -f {} \;
	(cd $(PERL-MIME-BASE64_IPK_DIR)$(TARGET_PREFIX)/lib/perl5 ; \
		find . -name '*.so' -exec chmod +w {} \; ; \
		find . -name '*.so' -exec $(STRIP_COMMAND) {} \; ; \
		find . -name '*.so' -exec chmod -w {} \; ; \
	)
	find $(PERL-MIME-BASE64_IPK_DIR)$(TARGET_PREFIX) -type d -exec chmod go+rx {} \;
