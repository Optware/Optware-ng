...
#
PY-XDG_IPK_VERSION=1

#
# PY-XDG_CONFFILES should be a list of user-editable files
#PY-XDG_CONFFILES=/opt/etc/py-xdg.conf /opt/etc/init.d/SXXpy-xdg
...
#
# Binaries should be installed into $(PY-XDG_IPK_DIR)/opt/sbin or $(PY-XDG_IPK_DIR)/opt/bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(PY-XDG_IPK_DIR)/opt/{lib,include}
# Configuration files should be installed in $(PY-XDG_IPK_DIR)/opt/etc/py-xdg/...
# Documentation files should be installed in $(PY-XDG_IPK_DIR)/opt/doc/py-xdg/...
# Daemon startup scripts should be installed in $(PY-XDG_IPK_DIR)/opt/etc/init.d/S??py-xdg
#
...
$(PY24-XDG_IPK): $(PY-XDG_BUILD_DIR)/.built
	...
	$(HOST_STAGING_PREFIX)/bin/python2.4 setup.py install --root=$(PY24-XDG_IPK_DIR) --prefix=/opt)
	rm -f $(PY24-XDG_IPK_DIR)/opt/bin/easy_install
...
$(PY25-XDG_IPK): $(PY-XDG_BUILD_DIR)/.built
	...
	$(HOST_STAGING_PREFIX)/bin/python2.5 setup.py install --root=$(PY25-XDG_IPK_DIR) --prefix=/opt)
	rm -f $(PY25-XDG_IPK_DIR)/opt/bin/easy_install
...
$(PY26-XDG_IPK): $(PY-XDG_BUILD_DIR)/.built
	...
	$(HOST_STAGING_PREFIX)/bin/python2.6 setup.py install --root=$(PY26-XDG_IPK_DIR) --prefix=/opt)
#	rm -f $(PY26-XDG_IPK_DIR)/opt/bin/easy_install
...
$(PY27-XDG_IPK): $(PY-XDG_BUILD_DIR)/.built
	...
	$(HOST_STAGING_PREFIX)/bin/python2.7 setup.py install --root=$(PY27-XDG_IPK_DIR) --prefix=/opt)
	rm -f $(PY27-XDG_IPK_DIR)/opt/bin/easy_install
...
$(PY3-XDG_IPK): $(PY-XDG_BUILD_DIR)/.built
	...
	$(HOST_STAGING_PREFIX)/bin/python$(PYTHON3_VERSION_MAJOR) setup.py install --root=$(PY3-XDG_IPK_DIR) --prefix=/opt)
	rm -f $(PY3-XDG_IPK_DIR)/opt/bin/easy_install
