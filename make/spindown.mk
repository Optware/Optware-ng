...
#SPINDOWN_CONFFILES=$(TARGET_PREFIX)/etc/spindown.conf $(TARGET_PREFIX)/etc/init.d/SXXspindown
...
#
# Binaries should be installed into $(SPINDOWN_IPK_DIR)$(TARGET_PREFIX)/sbin or $(SPINDOWN_IPK_DIR)$(TARGET_PREFIX)/bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(SPINDOWN_IPK_DIR)$(TARGET_PREFIX)/{lib,include}
# Configuration files should be installed in $(SPINDOWN_IPK_DIR)$(TARGET_PREFIX)/etc/spindown/...
# Documentation files should be installed in $(SPINDOWN_IPK_DIR)$(TARGET_PREFIX)/doc/spindown/...
# Daemon startup scripts should be installed in $(SPINDOWN_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/S??spindown
#
...
$(SPINDOWN_IPK): $(SPINDOWN_BUILD_DIR)/.built
	...
	$(INSTALL) -d $(SPINDOWN_IPK_DIR)$(TARGET_PREFIX)/share/doc/spindown
	$(INSTALL) $(SPINDOWN_BUILD_DIR)/README $(SPINDOWN_IPK_DIR)$(TARGET_PREFIX)/share/doc/spindown/
#	$(INSTALL) -d $(SPINDOWN_IPK_DIR)$(TARGET_PREFIX)/etc/
#	$(INSTALL) -m 644 $(SPINDOWN_SOURCE_DIR)/spindown.conf $(SPINDOWN_IPK_DIR)$(TARGET_PREFIX)/etc/spindown.conf
#	$(INSTALL) -d $(SPINDOWN_IPK_DIR)$(TARGET_PREFIX)/etc/init.d
#	$(INSTALL) -m 755 $(SPINDOWN_SOURCE_DIR)/rc.spindown $(SPINDOWN_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/SXXspindown
#	sed -i -e '/^#!/aOPTWARE_TARGET=${OPTWARE_TARGET}' $(SPINDOWN_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/SXXspindown
