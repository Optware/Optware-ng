...
PERL-IO-SOCKET-SSL_SITE=http://$(PERL_CPAN_SITE)/CPAN/authors/id/S/SU/SULLR
PERL-IO-SOCKET-SSL_VERSION=2.012
...
PERL-IO-SOCKET-SSL_IPK_VERSION=3
...
$(DL_DIR)/$(PERL-IO-SOCKET-SSL_SOURCE):
	$(WGET) -P $(@D) $(PERL-IO-SOCKET-SSL_SITE)/$(@F) || \
	$(WGET) -P $(@D) $(FREEBSD_DISTFILES)/$(@F) || \
	$(WGET) -P $(@D) $(SOURCES_NLO_SITE)/$(@F)

perl-io-socket-ssl-source: $(DL_DIR)/$(PERL-IO-SOCKET-SSL_SOURCE) $(PERL-IO-SOCKET-SSL_PATCHES)

$(PERL-IO-SOCKET-SSL_BUILD_DIR)/.configured: $(DL_DIR)/$(PERL-IO-SOCKET-SSL_SOURCE) $(PERL-IO-SOCKET-SSL_PATCHES) make/perl-io-socket-ssl.mk
...
$(PERL-IO-SOCKET-SSL_IPK): $(PERL-IO-SOCKET-SSL_BUILD_DIR)/.built
	...
	find $(PERL-IO-SOCKET-SSL_IPK_DIR)$(TARGET_PREFIX) -name 'perllocal.pod' -exec rm -f {} \;
	(cd $(PERL-IO-SOCKET-SSL_IPK_DIR)$(TARGET_PREFIX)/lib/perl5 ; \
		find . -name '*.so' -exec chmod +w {} \; ; \
		find . -name '*.so' -exec $(STRIP_COMMAND) {} \; ; \
		find . -name '*.so' -exec chmod -w {} \; ; \
	)
	find $(PERL-IO-SOCKET-SSL_IPK_DIR)$(TARGET_PREFIX) -type d -exec chmod go+rx {} \;
