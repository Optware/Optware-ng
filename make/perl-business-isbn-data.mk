...
PERL-BUSINESS-ISBN-DATA_SITE=http://$(PERL_CPAN_SITE)/CPAN/authors/id/B/BD/BDFOY
...
PERL-BUSINESS-ISBN-DATA_IPK_VERSION=3
...
$(DL_DIR)/$(PERL-BUSINESS-ISBN-DATA_SOURCE):
	$(WGET) -P $(@D) $(PERL-BUSINESS-ISBN-DATA_SITE)/$(@F) || \
	$(WGET) -P $(@D) $(FREEBSD_DISTFILES)/$(@F) || \
	$(WGET) -P $(@D) $(SOURCES_NLO_SITE)/$(@F)

perl-business-isbn-data-source: $(DL_DIR)/$(PERL-BUSINESS-ISBN-DATA_SOURCE) $(PERL-BUSINESS-ISBN-DATA_PATCHES)

$(PERL-BUSINESS-ISBN-DATA_BUILD_DIR)/.configured: $(DL_DIR)/$(PERL-BUSINESS-ISBN-DATA_SOURCE) $(PERL-BUSINESS-ISBN-DATA_PATCHES) make/perl-business-isbn-data.mk
	$(MAKE) perl-stage
...
$(PERL-BUSINESS-ISBN-DATA_IPK): $(PERL-BUSINESS-ISBN-DATA_BUILD_DIR)/.built
	...
	find $(PERL-BUSINESS-ISBN-DATA_IPK_DIR)$(TARGET_PREFIX) -name 'perllocal.pod' -exec rm -f {} \;
	(cd $(PERL-BUSINESS-ISBN-DATA_IPK_DIR)$(TARGET_PREFIX)/lib/perl5 ; \
		find . -name '*.so' -exec chmod +w {} \; ; \
		find . -name '*.so' -exec $(STRIP_COMMAND) {} \; ; \
		find . -name '*.so' -exec chmod -w {} \; ; \
	)
	find $(PERL-BUSINESS-ISBN-DATA_IPK_DIR)$(TARGET_PREFIX) -type d -exec chmod go+rx {} \;
