...
PERL-SYS-SYSCALL_SITE=http://$(PERL_CPAN_SITE)/CPAN/authors/id/B/BR/BRADFITZ
...
PERL-SYS-SYSCALL_IPK_VERSION=3
...
$(DL_DIR)/$(PERL-SYS-SYSCALL_SOURCE):
	$(WGET) -P $(@D) $(PERL-SYS-SYSCALL_SITE)/$(@F) || \
	$(WGET) -P $(@D) $(FREEBSD_DISTFILES)/$(@F) || \
	$(WGET) -P $(@D) $(SOURCES_NLO_SITE)/$(@F)

perl-sys-syscall-source: $(DL_DIR)/$(PERL-SYS-SYSCALL_SOURCE) $(PERL-SYS-SYSCALL_PATCHES)

$(PERL-SYS-SYSCALL_BUILD_DIR)/.configured: $(DL_DIR)/$(PERL-SYS-SYSCALL_SOURCE) $(PERL-SYS-SYSCALL_PATCHES) make/perl-sys-syscall.mk
	make perl-stage
...
$(PERL-SYS-SYSCALL_IPK): $(PERL-SYS-SYSCALL_BUILD_DIR)/.built
	...
	find $(PERL-SYS-SYSCALL_IPK_DIR)$(TARGET_PREFIX) -name 'perllocal.pod' -exec rm -f {} \;
	(cd $(PERL-SYS-SYSCALL_IPK_DIR)$(TARGET_PREFIX)/lib/perl5 ; \
		find . -name '*.so' -exec chmod +w {} \; ; \
		find . -name '*.so' -exec $(STRIP_COMMAND) {} \; ; \
		find . -name '*.so' -exec chmod -w {} \; ; \
	)
	find $(PERL-SYS-SYSCALL_IPK_DIR)$(TARGET_PREFIX) -type d -exec chmod go+rx {} \;
