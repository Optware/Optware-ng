...
PERL-ARCHIVE-ZIP_SITE=http://$(PERL_CPAN_SITE)/CPAN/authors/id/S/SM/SMPETERS
...
PERL-ARCHIVE-ZIP_IPK_VERSION=5
...
$(DL_DIR)/$(PERL-ARCHIVE-ZIP_SOURCE):
	$(WGET) -P $(@D) $(PERL-ARCHIVE-ZIP_SITE)/$(@F) || \
	$(WGET) -P $(@D) $(FREEBSD_DISTFILES)/$(@F) || \
	$(WGET) -P $(@D) $(SOURCES_NLO_SITE)/$(@F)

perl-archive-zip-source: $(DL_DIR)/$(PERL-ARCHIVE-ZIP_SOURCE) $(PERL-ARCHIVE-ZIP_PATCHES)

$(PERL-ARCHIVE-ZIP_BUILD_DIR)/.configured: $(DL_DIR)/$(PERL-ARCHIVE-ZIP_SOURCE) $(PERL-ARCHIVE-ZIP_PATCHES) make/perl-archive-zip.mk
...
$(PERL-ARCHIVE-ZIP_IPK): $(PERL-ARCHIVE-ZIP_BUILD_DIR)/.built
	...
	perl -pi -e 's|$(PERL_HOSTPERL)|$(TARGET_PREFIX)/bin/perl|g' $(PERL-ARCHIVE-ZIP_IPK_DIR)/*
	find $(PERL-ARCHIVE-ZIP_IPK_DIR)$(TARGET_PREFIX) -name 'perllocal.pod' -exec rm -f {} \;
	(cd $(PERL-ARCHIVE-ZIP_IPK_DIR)$(TARGET_PREFIX)/lib/perl5 ; \
		find . -name '*.so' -exec chmod +w {} \; ; \
		find . -name '*.so' -exec $(STRIP_COMMAND) {} \; ; \
		find . -name '*.so' -exec chmod -w {} \; ; \
	)
	find $(PERL-ARCHIVE-ZIP_IPK_DIR)$(TARGET_PREFIX) -type d -exec chmod go+rx {} \;
