...
PERL-UNICODE-STRING_SITE=http://$(PERL_CPAN_SITE)/CPAN/authors/id/G/GA/GAAS
...
PERL-UNICODE-STRING_IPK_VERSION=3
...
$(DL_DIR)/$(PERL-UNICODE-STRING_SOURCE):
	$(WGET) -P $(@D) $(PERL-UNICODE-STRING_SITE)/$(@F) || \
	$(WGET) -P $(@D) $(FREEBSD_DISTFILES)/$(@F) || \
	$(WGET) -P $(@D) $(SOURCES_NLO_SITE)/$(@F)

perl-unicode-string-source: $(DL_DIR)/$(PERL-UNICODE-STRING_SOURCE) $(PERL-UNICODE-STRING_PATCHES)

$(PERL-UNICODE-STRING_BUILD_DIR)/.configured: $(DL_DIR)/$(PERL-UNICODE-STRING_SOURCE) $(PERL-UNICODE-STRING_PATCHES) make/perl-unicode-string.mk
...
$(PERL-UNICODE-STRING_IPK): $(PERL-UNICODE-STRING_BUILD_DIR)/.built
	...
	find $(PERL-UNICODE-STRING_IPK_DIR)$(TARGET_PREFIX) -name 'perllocal.pod' -exec rm -f {} \;
	(cd $(PERL-UNICODE-STRING_IPK_DIR)$(TARGET_PREFIX)/lib/perl5 ; \
		find . -name '*.so' -exec chmod +w {} \; ; \
		find . -name '*.so' -exec $(STRIP_COMMAND) {} \; ; \
		find . -name '*.so' -exec chmod -w {} \; ; \
	)
	find $(PERL-UNICODE-STRING_IPK_DIR)$(TARGET_PREFIX) -type d -exec chmod go+rx {} \;
