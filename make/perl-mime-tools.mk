...
PERL-MIME-TOOLS_SITE=http://$(PERL_CPAN_SITE)/CPAN/authors/id/D/DS/DSKOLL
PERL-MIME-TOOLS_VERSION=5.507
...
$(DL_DIR)/$(PERL-MIME-TOOLS_SOURCE):
	$(WGET) -P $(@D) $(PERL-MIME-TOOLS_SITE)/$(@F) || \
	$(WGET) -P $(@D) $(FREEBSD_DISTFILES)/$(@F) || \
	$(WGET) -P $(@D) $(SOURCES_NLO_SITE)/$(@F)

perl-mime-tools-source: $(DL_DIR)/$(PERL-MIME-TOOLS_SOURCE) $(PERL-MIME-TOOLS_PATCHES)

$(PERL-MIME-TOOLS_BUILD_DIR)/.configured: $(DL_DIR)/$(PERL-MIME-TOOLS_SOURCE) $(PERL-MIME-TOOLS_PATCHES) make/perl-mime-tools.mk
...
$(PERL-MIME-TOOLS_IPK): $(PERL-MIME-TOOLS_BUILD_DIR)/.built
	...
	find $(PERL-MIME-TOOLS_IPK_DIR)$(TARGET_PREFIX) -name 'perllocal.pod' -exec rm -f {} \;
	(cd $(PERL-MIME-TOOLS_IPK_DIR)$(TARGET_PREFIX)/lib/perl5 ; \
		find . -name '*.so' -exec chmod +w {} \; ; \
		find . -name '*.so' -exec $(STRIP_COMMAND) {} \; ; \
		find . -name '*.so' -exec chmod -w {} \; ; \
	)
	find $(PERL-MIME-TOOLS_IPK_DIR)$(TARGET_PREFIX) -type d -exec chmod go+rx {} \;
