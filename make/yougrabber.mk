...
#
YOUGRABBER_IPK_VERSION=4

#
# YOUGRABBER_CONFFILES should be a list of user-editable files
#YOUGRABBER_CONFFILES=$(TARGET_PREFIX)/etc/yougrabber.conf $(TARGET_PREFIX)/etc/init.d/SXXyougrabber
...
#
# Binaries should be installed into $(YOUGRABBER_IPK_DIR)$(TARGET_PREFIX)/sbin or $(YOUGRABBER_IPK_DIR)$(TARGET_PREFIX)/bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(YOUGRABBER_IPK_DIR)$(TARGET_PREFIX)/{lib,include}
# Configuration files should be installed in $(YOUGRABBER_IPK_DIR)$(TARGET_PREFIX)/etc/yougrabber/...
# Documentation files should be installed in $(YOUGRABBER_IPK_DIR)$(TARGET_PREFIX)/doc/yougrabber/...
# Daemon startup scripts should be installed in $(YOUGRABBER_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/S??yougrabber
#
# You may need to patch your application to make it use these locations.
#
$(YOUGRABBER_IPK): $(YOUGRABBER_BUILD_DIR)/.built
	rm -rf $(YOUGRABBER_IPK_DIR) $(BUILD_DIR)/yougrabber_*_$(TARGET_ARCH).ipk
	$(INSTALL) -d $(YOUGRABBER_IPK_DIR)$(TARGET_PREFIX)/bin
	$(MAKE) -C $(YOUGRABBER_BUILD_DIR)/src install \
		PREFIX=$(YOUGRABBER_IPK_DIR)$(TARGET_PREFIX) \
		COPY=install
	$(STRIP_COMMAND) $(YOUGRABBER_IPK_DIR)$(TARGET_PREFIX)/bin/yg
	$(INSTALL) $(YOUGRABBER_BUILD_DIR)/CHANGELOG \
		...
		$(YOUGRABBER_IPK_DIR)$(TARGET_PREFIX)/share/doc/yougrabber
