...
PERL-HTML-TAGSET_SITE=http://$(PERL_CPAN_SITE)/CPAN/authors/id/P/PE/PETDANCE
PERL-HTML-TAGSET_VERSION=3.20
...
$(DL_DIR)/$(PERL-HTML-TAGSET_SOURCE):
	$(WGET) -P $(@D) $(PERL-HTML-TAGSET_SITE)/$(@F) || \
	$(WGET) -P $(@D) $(FREEBSD_DISTFILES)/$(@F) || \
	$(WGET) -P $(@D) $(SOURCES_NLO_SITE)/$(@F)

perl-html-tagset-source: $(DL_DIR)/$(PERL-HTML-TAGSET_SOURCE) $(PERL-HTML-TAGSET_PATCHES)

$(PERL-HTML-TAGSET_BUILD_DIR)/.configured: $(DL_DIR)/$(PERL-HTML-TAGSET_SOURCE) $(PERL-HTML-TAGSET_PATCHES) make/perl-html-tagset.mk
	make perl-stage
...
$(PERL-HTML-TAGSET_IPK): $(PERL-HTML-TAGSET_BUILD_DIR)/.built
	...
	find $(PERL-HTML-TAGSET_IPK_DIR)$(TARGET_PREFIX) -name 'perllocal.pod' -exec rm -f {} \;
	(cd $(PERL-HTML-TAGSET_IPK_DIR)$(TARGET_PREFIX)/lib/perl5 ; \
		find . -name '*.so' -exec chmod +w {} \; ; \
		find . -name '*.so' -exec $(STRIP_COMMAND) {} \; ; \
		find . -name '*.so' -exec chmod -w {} \; ; \
	)
	find $(PERL-HTML-TAGSET_IPK_DIR)$(TARGET_PREFIX) -type d -exec chmod go+rx {} \;
