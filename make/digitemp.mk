...
# DIGITEMP_CONFFILES=$(TARGET_PREFIX)/etc/digitemp.conf $(TARGET_PREFIX)/etc/init.d/SXXdigitemp
...
#
# Binaries should be installed into $(DIGITEMP_IPK_DIR)$(TARGET_PREFIX)/sbin or $(DIGITEMP_IPK_DIR)$(TARGET_PREFIX)/bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(DIGITEMP_IPK_DIR)$(TARGET_PREFIX)/{lib,include}
# Configuration files should be installed in $(DIGITEMP_IPK_DIR)$(TARGET_PREFIX)/etc/digitemp/...
# Documentation files should be installed in $(DIGITEMP_IPK_DIR)$(TARGET_PREFIX)/doc/digitemp/...
# Daemon startup scripts should be installed in $(DIGITEMP_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/S??digitemp
#
# You may need to patch your application to make it use these locations.
#
$(DIGITEMP_IPK): $(DIGITEMP_BUILD_DIR)/.built
	rm -rf $(DIGITEMP_IPK_DIR) $(BUILD_DIR)/digitemp_*_$(TARGET_ARCH).ipk
	$(INSTALL) -d $(DIGITEMP_IPK_DIR)$(TARGET_PREFIX)/bin
	$(INSTALL) -m 755 $(DIGITEMP_BUILD_DIR)/digitemp_DS9097 $(DIGITEMP_IPK_DIR)$(TARGET_PREFIX)/bin
	$(INSTALL) -m 755 $(DIGITEMP_BUILD_DIR)/digitemp_DS9097U $(DIGITEMP_IPK_DIR)$(TARGET_PREFIX)/bin
	$(INSTALL) -m 755 $(DIGITEMP_BUILD_DIR)/digitemp_DS2490 $(DIGITEMP_IPK_DIR)$(TARGET_PREFIX)/bin
	$(STRIP_COMMAND) $(DIGITEMP_IPK_DIR)$(TARGET_PREFIX)/bin/digitemp_DS9097
	$(STRIP_COMMAND) $(DIGITEMP_IPK_DIR)$(TARGET_PREFIX)/bin/digitemp_DS9097U
	$(STRIP_COMMAND) $(DIGITEMP_IPK_DIR)$(TARGET_PREFIX)/bin/digitemp_DS2490
