...
#RCS_CONFFILES=$(TARGET_PREFIX)/etc/rcs.conf $(TARGET_PREFIX)/etc/init.d/SXXrcs
...
ifeq ($(OPTWARE_TARGET),wl500g)
	$(INSTALL) -m 644 $(RCS_SOURCE_DIR)/wiley-src-conf.h $(RCS_BUILD_DIR)/src/conf.h
else
	$(INSTALL) -m 644 $(RCS_SOURCE_DIR)/slug-src-conf.h $(RCS_BUILD_DIR)/src/conf.h
endif
endif
...
#
# Binaries should be installed into $(RCS_IPK_DIR)$(TARGET_PREFIX)/sbin or $(RCS_IPK_DIR)$(TARGET_PREFIX)/bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(RCS_IPK_DIR)$(TARGET_PREFIX)/{lib,include}
# Configuration files should be installed in $(RCS_IPK_DIR)$(TARGET_PREFIX)/etc/rcs/...
# Documentation files should be installed in $(RCS_IPK_DIR)$(TARGET_PREFIX)/doc/rcs/...
# Daemon startup scripts should be installed in $(RCS_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/S??rcs
#
# You may need to patch your application to make it use these locations.
#
$(RCS_IPK): $(RCS_BUILD_DIR)/.built
	rm -rf $(RCS_IPK_DIR) $(BUILD_DIR)/rcs_*_$(TARGET_ARCH).ipk
	$(MAKE) -C $(RCS_BUILD_DIR) DESTDIR=$(RCS_IPK_DIR) prefix=$(RCS_IPK_DIR)$(TARGET_PREFIX) install
	$(STRIP_COMMAND) $(RCS_IPK_DIR)$(TARGET_PREFIX)/bin/*
