...
$(PERL-URI_IPK): $(PERL-URI_BUILD_DIR)/.built
	...
	find $(PERL-URI_IPK_DIR)$(TARGET_PREFIX) -name 'perllocal.pod' -exec rm -f {} \;
	(cd $(PERL-URI_IPK_DIR)$(TARGET_PREFIX)/lib/perl5 ; \
		find . -name '*.so' -exec chmod +w {} \; ; \
		find . -name '*.so' -exec $(STRIP_COMMAND) {} \; ; \
		find . -name '*.so' -exec chmod -w {} \; ; \
	)
	find $(PERL-URI_IPK_DIR)$(TARGET_PREFIX) -type d -exec chmod go+rx {} \;
