...
PERL-POE-XS-QUEUE-ARRAY_IPK_VERSION=3
...
$(PERL-POE-XS-QUEUE-ARRAY_BUILD_DIR)/.configured: $(DL_DIR)/$(PERL-POE-XS-QUEUE-ARRAY_SOURCE) $(PERL-POE-XS-QUEUE-ARRAY_PATCHES) make/perl-poe-xs-queue-array.mk
#	$(MAKE) <foo>-stage
...
$(PERL-POE-XS-QUEUE-ARRAY_IPK): $(PERL-POE-XS-QUEUE-ARRAY_BUILD_DIR)/.built
	...
	find $(PERL-POE-XS-QUEUE-ARRAY_IPK_DIR)$(TARGET_PREFIX) -name 'perllocal.pod' -exec rm -f {} \;
	(cd $(PERL-POE-XS-QUEUE-ARRAY_IPK_DIR)$(TARGET_PREFIX)/lib/perl5 ; \
		find . -name '*.so' -exec chmod +w {} \; ; \
		find . -name '*.so' -exec $(STRIP_COMMAND) {} \; ; \
		find . -name '*.so' -exec chmod -w {} \; ; \
	)
	find $(PERL-POE-XS-QUEUE-ARRAY_IPK_DIR)$(TARGET_PREFIX) -type d -exec chmod go+rx {} \;
