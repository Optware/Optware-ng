...
$(PERL-LIBWWW_IPK): $(PERL-LIBWWW_BUILD_DIR)/.built
	...
	find $(PERL-LIBWWW_IPK_DIR)$(TARGET_PREFIX) -name 'perllocal.pod' -exec rm -f {} \;
	(cd $(PERL-LIBWWW_IPK_DIR)$(TARGET_PREFIX)/lib/perl5 ; \
		find . -name '*.so' -exec chmod +w {} \; ; \
		find . -name '*.so' -exec $(STRIP_COMMAND) {} \; ; \
		find . -name '*.so' -exec chmod -w {} \; ; \
	)
	find $(PERL-LIBWWW_IPK_DIR)$(TARGET_PREFIX) -type d -exec chmod go+rx {} \;
