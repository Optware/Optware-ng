...
#POLIPO_CONFFILES=$(TARGET_PREFIX)/etc/polipo.conf $(TARGET_PREFIX)/etc/init.d/SXXpolipo
...
$(POLIPO_BUILD_DIR)/.built: $(POLIPO_BUILD_DIR)/.configured
	...
	$(MAKE) -C $(@D) \
		...
		LDFLAGS="$(STAGING_LDFLAGS) $(POLIPO_LDFLAGS)" \
		PREFIX=$(TARGET_PREFIX) \
		LOCAL_ROOT=$(TARGET_PREFIX)/share/polipo/www \
		DISK_CACHE_ROOT=$(TARGET_PREFIX)/var/cache/polipo \
		;
	touch $@
...
#
# Binaries should be installed into $(POLIPO_IPK_DIR)$(TARGET_PREFIX)/sbin or $(POLIPO_IPK_DIR)$(TARGET_PREFIX)/bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(POLIPO_IPK_DIR)$(TARGET_PREFIX)/{lib,include}
# Configuration files should be installed in $(POLIPO_IPK_DIR)$(TARGET_PREFIX)/etc/polipo/...
# Documentation files should be installed in $(POLIPO_IPK_DIR)$(TARGET_PREFIX)/doc/polipo/...
# Daemon startup scripts should be installed in $(POLIPO_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/S??polipo
#
...
$(POLIPO_IPK): $(POLIPO_BUILD_DIR)/.built
	...
	$(MAKE) -C $(POLIPO_BUILD_DIR) install \
		...
		DISK_CACHE_ROOT=$(TARGET_PREFIX)/var/cache/polipo \
		;
	$(STRIP_COMMAND) $(POLIPO_IPK_DIR)$(TARGET_PREFIX)/*bin/*
	rm -f $(POLIPO_IPK_DIR)$(TARGET_PREFIX)/info/dir*
	$(INSTALL) -d $(POLIPO_IPK_DIR)$(TARGET_PREFIX)/var/cache/polipo
