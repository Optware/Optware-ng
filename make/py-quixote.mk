...
#PY-QUIXOTE_CONFFILES=$(TARGET_PREFIX)/etc/py-quixote.conf $(TARGET_PREFIX)/etc/init.d/SXXpy-quixote
...
#
# Binaries should be installed into $(PY-QUIXOTE_IPK_DIR)$(TARGET_PREFIX)/sbin or $(PY-QUIXOTE_IPK_DIR)$(TARGET_PREFIX)/bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(PY-QUIXOTE_IPK_DIR)$(TARGET_PREFIX)/{lib,include}
# Configuration files should be installed in $(PY-QUIXOTE_IPK_DIR)$(TARGET_PREFIX)/etc/py-quixote/...
# Documentation files should be installed in $(PY-QUIXOTE_IPK_DIR)$(TARGET_PREFIX)/doc/py-quixote/...
# Daemon startup scripts should be installed in $(PY-QUIXOTE_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/S??py-quixote
#
...
$(PY25-QUIXOTE_IPK): $(PY-QUIXOTE_BUILD_DIR)/.built
	...
	(cd $(PY-QUIXOTE_BUILD_DIR)/2.5; \
	    ...
	    --root=$(PY25-QUIXOTE_IPK_DIR) --prefix=$(TARGET_PREFIX); \
	)
	-$(STRIP_COMMAND) `find $(PY25-QUIXOTE_IPK_DIR)$(TARGET_PREFIX)/lib/python2.5/site-packages/quixote -name '*.so'`
...
$(PY26-QUIXOTE_IPK): $(PY-QUIXOTE_BUILD_DIR)/.built
	...
	(cd $(PY-QUIXOTE_BUILD_DIR)/2.6; \
	    ...
	    --root=$(PY26-QUIXOTE_IPK_DIR) --prefix=$(TARGET_PREFIX); \
	)
	-$(STRIP_COMMAND) `find $(PY26-QUIXOTE_IPK_DIR)$(TARGET_PREFIX)/lib/python2.6/site-packages/quixote -name '*.so'`
