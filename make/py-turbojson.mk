...
#PY-TURBOJSON_CONFFILES=$(TARGET_PREFIX)/etc/py-turbojson.conf $(TARGET_PREFIX)/etc/init.d/SXXpy-turbojson
...
#
# Binaries should be installed into $(PY-TURBOJSON_IPK_DIR)$(TARGET_PREFIX)/sbin or $(PY-TURBOJSON_IPK_DIR)$(TARGET_PREFIX)/bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(PY-TURBOJSON_IPK_DIR)$(TARGET_PREFIX)/{lib,include}
# Configuration files should be installed in $(PY-TURBOJSON_IPK_DIR)$(TARGET_PREFIX)/etc/py-turbojson/...
# Documentation files should be installed in $(PY-TURBOJSON_IPK_DIR)$(TARGET_PREFIX)/doc/py-turbojson/...
# Daemon startup scripts should be installed in $(PY-TURBOJSON_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/S??py-turbojson
#
...
$(PY24-TURBOJSON_IPK): $(PY-TURBOJSON_BUILD_DIR)/.built
	...
	(cd $(PY-TURBOJSON_BUILD_DIR)/2.4; \
		...
		--root=$(PY24-TURBOJSON_IPK_DIR) --prefix=$(TARGET_PREFIX))
...
$(PY25-TURBOJSON_IPK): $(PY-TURBOJSON_BUILD_DIR)/.built
	...
	(cd $(PY-TURBOJSON_BUILD_DIR)/2.5; \
		...
		--root=$(PY25-TURBOJSON_IPK_DIR) --prefix=$(TARGET_PREFIX))
