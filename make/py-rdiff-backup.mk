...
#PY-RDIFF-BACKUP_CONFFILES=$(TARGET_PREFIX)/etc/py-rdiff-backup.conf $(TARGET_PREFIX)/etc/init.d/SXXpy-rdiff-backup
...
#
# Binaries should be installed into $(PY-RDIFF-BACKUP_IPK_DIR)$(TARGET_PREFIX)/sbin or $(PY-RDIFF-BACKUP_IPK_DIR)$(TARGET_PREFIX)/bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(PY-RDIFF-BACKUP_IPK_DIR)$(TARGET_PREFIX)/{lib,include}
# Configuration files should be installed in $(PY-RDIFF-BACKUP_IPK_DIR)$(TARGET_PREFIX)/etc/py-rdiff-backup/...
# Documentation files should be installed in $(PY-RDIFF-BACKUP_IPK_DIR)$(TARGET_PREFIX)/doc/py-rdiff-backup/...
# Daemon startup scripts should be installed in $(PY-RDIFF-BACKUP_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/S??py-rdiff-backup
#
...
$(PY25-RDIFF-BACKUP_IPK): $(PY-RDIFF-BACKUP_BUILD_DIR)/.built
	...
	(cd $(PY-RDIFF-BACKUP_BUILD_DIR)/2.5; \
	 CC='$(TARGET_CC)' LDSHARED='$(TARGET_CC) -shared' \
	    ...
	    --root=$(PY25-RDIFF-BACKUP_IPK_DIR) --prefix=$(TARGET_PREFIX); \
	)
	$(STRIP_COMMAND) $(PY25-RDIFF-BACKUP_IPK_DIR)$(TARGET_PREFIX)/lib/python2.5/site-packages/*/*.so
...
$(PY26-RDIFF-BACKUP_IPK): $(PY-RDIFF-BACKUP_BUILD_DIR)/.built
	...
	(cd $(PY-RDIFF-BACKUP_BUILD_DIR)/2.6; \
	 CC='$(TARGET_CC)' LDSHARED='$(TARGET_CC) -shared' \
	    ...
	    --root=$(PY26-RDIFF-BACKUP_IPK_DIR) --prefix=$(TARGET_PREFIX); \
	)
	$(STRIP_COMMAND) $(PY26-RDIFF-BACKUP_IPK_DIR)$(TARGET_PREFIX)/lib/python2.6/site-packages/*/*.so
	rm -rf $(PY26-RDIFF-BACKUP_IPK_DIR)$(TARGET_PREFIX)/share
	for f in $(PY26-RDIFF-BACKUP_IPK_DIR)$(TARGET_PREFIX)/*bin/*; \
