...
PERL-CONVERT-BINHEX_SITE=http://$(PERL_CPAN_SITE)/CPAN/authors/id/E/ER/ERYQ
...
PERL-CONVERT-BINHEX_IPK_VERSION=3
...
$(DL_DIR)/$(PERL-CONVERT-BINHEX_SOURCE):
	$(WGET) -P $(@D) $(PERL-CONVERT-BINHEX_SITE)/$(@F) || \
	$(WGET) -P $(@D) $(FREEBSD_DISTFILES)/$(@F) || \
	$(WGET) -P $(@D) $(SOURCES_NLO_SITE)/$(@F)

perl-convert-binhex-source: $(DL_DIR)/$(PERL-CONVERT-BINHEX_SOURCE) $(PERL-CONVERT-BINHEX_PATCHES)

$(PERL-CONVERT-BINHEX_BUILD_DIR)/.configured: $(DL_DIR)/$(PERL-CONVERT-BINHEX_SOURCE) $(PERL-CONVERT-BINHEX_PATCHES) make/perl-convert-binhex.mk
...
$(PERL-CONVERT-BINHEX_IPK): $(PERL-CONVERT-BINHEX_BUILD_DIR)/.built
	...
	find $(PERL-CONVERT-BINHEX_IPK_DIR)$(TARGET_PREFIX) -name 'perllocal.pod' -exec rm -f {} \;
	(cd $(PERL-CONVERT-BINHEX_IPK_DIR)$(TARGET_PREFIX)/lib/perl5 ; \
		find . -name '*.so' -exec chmod +w {} \; ; \
		find . -name '*.so' -exec $(STRIP_COMMAND) {} \; ; \
		find . -name '*.so' -exec chmod -w {} \; ; \
	)
	find $(PERL-CONVERT-BINHEX_IPK_DIR)$(TARGET_PREFIX) -type d -exec chmod go+rx {} \;
