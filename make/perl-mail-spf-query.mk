...
PERL-MAIL-SPF-QUERY_SITE=http://$(PERL_CPAN_SITE)/CPAN/authors/id/J/JM/JMEHNLE/mail-spf-query
...
PERL-MAIL-SPF-QUERY_IPK_VERSION=3
...
$(DL_DIR)/$(PERL-MAIL-SPF-QUERY_SOURCE):
	$(WGET) -P $(@D) $(PERL-MAIL-SPF-QUERY_SITE)/$(@F) || \
	$(WGET) -P $(@D) $(FREEBSD_DISTFILES)/$(@F) || \
	$(WGET) -P $(@D) $(SOURCES_NLO_SITE)/$(@F)

perl-mail-spf-query-source: $(DL_DIR)/$(PERL-MAIL-SPF-QUERY_SOURCE) $(PERL-MAIL-SPF-QUERY_PATCHES)

$(PERL-MAIL-SPF-QUERY_BUILD_DIR)/.configured: $(DL_DIR)/$(PERL-MAIL-SPF-QUERY_SOURCE) $(PERL-MAIL-SPF-QUERY_PATCHES) make/perl-mail-spf-query.mk
...
$(PERL-MAIL-SPF-QUERY_IPK): $(PERL-MAIL-SPF-QUERY_BUILD_DIR)/.built
	...
	find $(PERL-MAIL-SPF-QUERY_IPK_DIR)$(TARGET_PREFIX) -name 'perllocal.pod' -exec rm -f {} \;
	(cd $(PERL-MAIL-SPF-QUERY_IPK_DIR)$(TARGET_PREFIX)/lib/perl5 ; \
		find . -name '*.so' -exec chmod +w {} \; ; \
		find . -name '*.so' -exec $(STRIP_COMMAND) {} \; ; \
		find . -name '*.so' -exec chmod -w {} \; ; \
	)
	find $(PERL-MAIL-SPF-QUERY_IPK_DIR)$(TARGET_PREFIX) -type d -exec chmod go+rx {} \;
