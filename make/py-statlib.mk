...
#PY-STATLIB_CONFFILES=$(TARGET_PREFIX)/etc/py-statlib.conf $(TARGET_PREFIX)/etc/init.d/SXXpy-statlib
...
#
# Binaries should be installed into $(PY-STATLIB_IPK_DIR)$(TARGET_PREFIX)/sbin or $(PY-STATLIB_IPK_DIR)$(TARGET_PREFIX)/bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(PY-STATLIB_IPK_DIR)$(TARGET_PREFIX)/{lib,include}
# Configuration files should be installed in $(PY-STATLIB_IPK_DIR)$(TARGET_PREFIX)/etc/py-statlib/...
# Documentation files should be installed in $(PY-STATLIB_IPK_DIR)$(TARGET_PREFIX)/doc/py-statlib/...
# Daemon startup scripts should be installed in $(PY-STATLIB_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/S??py-statlib
#
...
$(PY25-STATLIB_IPK): $(PY-STATLIB_BUILD_DIR)/.built
	...
	(cd $(PY-STATLIB_BUILD_DIR)/2.5; \
	    $(HOST_STAGING_PREFIX)/bin/python2.5 setup.py install --root=$(PY25-STATLIB_IPK_DIR) --prefix=$(TARGET_PREFIX); \
	)
#	$(STRIP_COMMAND) $(PY25-STATLIB_IPK_DIR)$(TARGET_PREFIX)/lib/python2.5/site-packages/bzrlib/*.so
...
$(PY26-STATLIB_IPK): $(PY-STATLIB_BUILD_DIR)/.built
	...
	(cd $(PY-STATLIB_BUILD_DIR)/2.6; \
	    $(HOST_STAGING_PREFIX)/bin/python2.6 setup.py install --root=$(PY26-STATLIB_IPK_DIR) --prefix=$(TARGET_PREFIX); \
	)
#	$(STRIP_COMMAND) $(PY26-STATLIB_IPK_DIR)$(TARGET_PREFIX)/lib/python2.6/site-packages/bzrlib/*.so
