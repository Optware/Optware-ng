...
PERL-DANGA-SOCKET_IPK_VERSION=3
...
$(PERL-DANGA-SOCKET_BUILD_DIR)/.configured: $(DL_DIR)/$(PERL-DANGA-SOCKET_SOURCE) $(PERL-DANGA-SOCKET_PATCHES) make/perl-danga-socket.mk
	make perl-stage
...
$(PERL-DANGA-SOCKET_IPK): $(PERL-DANGA-SOCKET_BUILD_DIR)/.built
	...
	find $(PERL-DANGA-SOCKET_IPK_DIR)$(TARGET_PREFIX) -name 'perllocal.pod' -exec rm -f {} \;
	(cd $(PERL-DANGA-SOCKET_IPK_DIR)$(TARGET_PREFIX)/lib/perl5 ; \
		find . -name '*.so' -exec chmod +w {} \; ; \
		find . -name '*.so' -exec $(STRIP_COMMAND) {} \; ; \
		find . -name '*.so' -exec chmod -w {} \; ; \
	)
	find $(PERL-DANGA-SOCKET_IPK_DIR)$(TARGET_PREFIX) -type d -exec chmod go+rx {} \;
