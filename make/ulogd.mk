...
#ULOGD_CONFFILES=$(TARGET_PREFIX)/etc/ulogd.conf $(TARGET_PREFIX)/etc/init.d/SXXulogd
...
$(ULOGD_BUILD_DIR)/.built: $(ULOGD_BUILD_DIR)/.configured
	rm -f $@
	$(MAKE) -C $(@D) \
		LD="$(TARGET_CC) -L$(STAGING_LIB_DIR) -Wl,-rpath -Wl,$(TARGET_PREFIX)/lib" \
		AR=$(TARGET_AR)
	touch $@
...
#
# Binaries should be installed into $(ULOGD_IPK_DIR)$(TARGET_PREFIX)/sbin or $(ULOGD_IPK_DIR)$(TARGET_PREFIX)/bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(ULOGD_IPK_DIR)$(TARGET_PREFIX)/{lib,include}
# Configuration files should be installed in $(ULOGD_IPK_DIR)$(TARGET_PREFIX)/etc/ulogd/...
# Documentation files should be installed in $(ULOGD_IPK_DIR)$(TARGET_PREFIX)/doc/ulogd/...
# Daemon startup scripts should be installed in $(ULOGD_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/S??ulogd
#
...
$(ULOGD_IPK): $(ULOGD_BUILD_DIR)/.built
	...
	$(STRIP_COMMAND) $(ULOGD_IPK_DIR)$(TARGET_PREFIX)/sbin/* $(ULOGD_IPK_DIR)$(TARGET_PREFIX)/lib/ulogd/*.so
