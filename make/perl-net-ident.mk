...
$(PERL-NET-IDENT_BUILD_DIR)/.configured: $(DL_DIR)/$(PERL-NET-IDENT_SOURCE) $(PERL-NET-IDENT_PATCHES) make/perl-net-ident.mk
	make perl-stage
...
$(PERL-NET-IDENT_IPK): $(PERL-NET-IDENT_BUILD_DIR)/.built
	...
	find $(PERL-NET-IDENT_IPK_DIR)$(TARGET_PREFIX) -name 'perllocal.pod' -exec rm -f {} \;
	(cd $(PERL-NET-IDENT_IPK_DIR)$(TARGET_PREFIX)/lib/perl5 ; \
		find . -name '*.so' -exec chmod +w {} \; ; \
		find . -name '*.so' -exec $(STRIP_COMMAND) {} \; ; \
		find . -name '*.so' -exec chmod -w {} \; ; \
	)
	find $(PERL-NET-IDENT_IPK_DIR)$(TARGET_PREFIX) -type d -exec chmod go+rx {} \;
