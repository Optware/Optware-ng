...
PERL-TERM-READKEY_SITE=http://$(PERL_CPAN_SITE)/CPAN/authors/id/J/JS/JSTOWE
...
PERL-TERM-READKEY_IPK_VERSION=4
...
$(DL_DIR)/$(PERL-TERM-READKEY_SOURCE):
	$(WGET) -P $(@D) $(PERL-TERM-READKEY_SITE)/$(@F) || \
	$(WGET) -P $(@D) $(FREEBSD_DISTFILES)/$(@F) || \
	$(WGET) -P $(@D) $(SOURCES_NLO_SITE)/$(@F)

perl-term-readkey-source: $(DL_DIR)/$(PERL-TERM-READKEY_SOURCE) $(PERL-TERM-READKEY_PATCHES)

$(PERL-TERM-READKEY_BUILD_DIR)/.configured: $(DL_DIR)/$(PERL-TERM-READKEY_SOURCE) $(PERL-TERM-READKEY_PATCHES) make/perl-term-readkey.mk
	$(MAKE) perl-stage
...
$(PERL-TERM-READKEY_IPK): $(PERL-TERM-READKEY_BUILD_DIR)/.built
	...
	find $(PERL-TERM-READKEY_IPK_DIR)$(TARGET_PREFIX) -name 'perllocal.pod' -exec rm -f {} \;
	(cd $(PERL-TERM-READKEY_IPK_DIR)$(TARGET_PREFIX)/lib/perl5 ; \
		find . -name '*.so' -exec chmod +w {} \; ; \
		find . -name '*.so' -exec $(STRIP_COMMAND) {} \; ; \
		find . -name '*.so' -exec chmod -w {} \; ; \
	)
	find $(PERL-TERM-READKEY_IPK_DIR)$(TARGET_PREFIX) -type d -exec chmod go+rx {} \;
