...
#LZMA_CONFFILES=$(TARGET_PREFIX)/etc/lzma.conf $(TARGET_PREFIX)/etc/init.d/SXXlzma
...
#
# Binaries should be installed into $(LZMA_IPK_DIR)$(TARGET_PREFIX)/sbin or $(LZMA_IPK_DIR)$(TARGET_PREFIX)/bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(LZMA_IPK_DIR)$(TARGET_PREFIX)/{lib,include}
# Configuration files should be installed in $(LZMA_IPK_DIR)$(TARGET_PREFIX)/etc/lzma/...
# Documentation files should be installed in $(LZMA_IPK_DIR)$(TARGET_PREFIX)/doc/lzma/...
# Daemon startup scripts should be installed in $(LZMA_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/S??lzma
#
...
#	$(INSTALL) -d $(LZMA_IPK_DIR)$(TARGET_PREFIX)/etc/
#	$(INSTALL) -m 644 $(LZMA_SOURCE_DIR)/lzma.conf $(LZMA_IPK_DIR)$(TARGET_PREFIX)/etc/lzma.conf
#	$(INSTALL) -d $(LZMA_IPK_DIR)$(TARGET_PREFIX)/etc/init.d
#	$(INSTALL) -m 755 $(LZMA_SOURCE_DIR)/rc.lzma $(LZMA_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/SXXlzma
#	sed -i -e '/^#!/aOPTWARE_TARGET=${OPTWARE_TARGET}' $(LZMA_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/SXXlzma
