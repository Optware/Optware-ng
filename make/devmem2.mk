...
#DEVMEM2_CONFFILES=$(TARGET_PREFIX)/etc/devmem2.conf $(TARGET_PREFIX)/etc/init.d/SXXdevmem2
...
#
# Binaries should be installed into $(DEVMEM2_IPK_DIR)$(TARGET_PREFIX)/sbin or $(DEVMEM2_IPK_DIR)$(TARGET_PREFIX)/bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(DEVMEM2_IPK_DIR)$(TARGET_PREFIX)/{lib,include}
# Configuration files should be installed in $(DEVMEM2_IPK_DIR)$(TARGET_PREFIX)/etc/devmem2/...
# Documentation files should be installed in $(DEVMEM2_IPK_DIR)$(TARGET_PREFIX)/doc/devmem2/...
# Daemon startup scripts should be installed in $(DEVMEM2_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/S??devmem2
#
# You may need to patch your application to make it use these locations.
#
$(DEVMEM2_IPK): $(DEVMEM2_BUILD_DIR)/.built
	rm -rf $(DEVMEM2_IPK_DIR) $(BUILD_DIR)/devmem2_*_$(TARGET_ARCH).ipk
	$(INSTALL) -d $(DEVMEM2_IPK_DIR)$(TARGET_PREFIX)/bin
	$(INSTALL) -m 755 $(<D)/devmem2 $(DEVMEM2_IPK_DIR)$(TARGET_PREFIX)/bin/devmem2
	$(STRIP_COMMAND) $(DEVMEM2_IPK_DIR)$(TARGET_PREFIX)/bin/devmem2
