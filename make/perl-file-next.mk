...
$(PERL-FILE-NEXT_BUILD_DIR)/.configured: $(DL_DIR)/$(PERL-FILE-NEXT_SOURCE) $(PERL-FILE-NEXT_PATCHES) make/perl-file-next.mk
...
$(PERL-FILE-NEXT_IPK): $(PERL-FILE-NEXT_BUILD_DIR)/.built
	...
	find $(PERL-FILE-NEXT_IPK_DIR)$(TARGET_PREFIX) -name 'perllocal.pod' -exec rm -f {} \;
	(cd $(PERL-FILE-NEXT_IPK_DIR)$(TARGET_PREFIX)/lib/perl5 ; \
		find . -name '*.so' -exec chmod +w {} \; ; \
		find . -name '*.so' -exec $(STRIP_COMMAND) {} \; ; \
		find . -name '*.so' -exec chmod -w {} \; ; \
	)
	find $(PERL-FILE-NEXT_IPK_DIR)$(TARGET_PREFIX) -type d -exec chmod go+rx {} \;
