...
#PY-WEATHERGET_CONFFILES=$(TARGET_PREFIX)/etc/py-weatherget.conf $(TARGET_PREFIX)/etc/init.d/SXXpy-weatherget
...
$(PY-WEATHERGET_BUILD_DIR)/.configured: $(DL_DIR)/$(PY-WEATHERGET_SOURCE) $(PY-WEATHERGET_PATCHES)
	...
	sed -i -e 's|/usr/|$(TARGET_PREFIX)/|g' $(BUILD_DIR)/$(PY-WEATHERGET_DIR)/setup.py
...
#
# Binaries should be installed into $(PY-WEATHERGET_IPK_DIR)$(TARGET_PREFIX)/sbin or $(PY-WEATHERGET_IPK_DIR)$(TARGET_PREFIX)/bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(PY-WEATHERGET_IPK_DIR)$(TARGET_PREFIX)/{lib,include}
# Configuration files should be installed in $(PY-WEATHERGET_IPK_DIR)$(TARGET_PREFIX)/etc/py-weatherget/...
# Documentation files should be installed in $(PY-WEATHERGET_IPK_DIR)$(TARGET_PREFIX)/doc/py-weatherget/...
# Daemon startup scripts should be installed in $(PY-WEATHERGET_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/S??py-weatherget
#
...
$(PY25-WEATHERGET_IPK) $(PY26-WEATHERGET_IPK) $(PY-WEATHERGET-DOC_IPK): $(PY-WEATHERGET_BUILD_DIR)/.built
	...
	# 2.5
	$(INSTALL) -d $(PY25-WEATHERGET_IPK_DIR)$(TARGET_PREFIX)/lib/python2.5/site-packages
	cd $(PY-WEATHERGET_BUILD_DIR)/2.5; \
	    $(HOST_STAGING_PREFIX)/bin/python2.5 setup.py install \
	    --root=$(PY25-WEATHERGET_IPK_DIR) --prefix=$(TARGET_PREFIX)
	rm -rf $(PY25-WEATHERGET_IPK_DIR)$(TARGET_PREFIX)/share
	$(MAKE) $(PY25-WEATHERGET_IPK_DIR)/CONTROL/control
	cd $(BUILD_DIR); $(IPKG_BUILD) $(PY25-WEATHERGET_IPK_DIR)
	# 2.6
	$(INSTALL) -d $(PY26-WEATHERGET_IPK_DIR)$(TARGET_PREFIX)/lib/python2.6/site-packages
	cd $(PY-WEATHERGET_BUILD_DIR)/2.6; \
	    $(HOST_STAGING_PREFIX)/bin/python2.6 setup.py install \
	    --root=$(PY26-WEATHERGET_IPK_DIR) --prefix=$(TARGET_PREFIX)
	for f in $(PY26-WEATHERGET_IPK_DIR)$(TARGET_PREFIX)/*bin/*; \
		do mv $$f `echo $$f | sed 's|$$|-py2.6|'`; done
	$(INSTALL) -d $(PY-WEATHERGET-DOC_IPK_DIR)$(TARGET_PREFIX)/
	mv $(PY26-WEATHERGET_IPK_DIR)$(TARGET_PREFIX)/share $(PY-WEATHERGET-DOC_IPK_DIR)$(TARGET_PREFIX)/
