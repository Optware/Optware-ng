...
$(CROSSTOOL-NATIVE_IPK): $(CROSSTOOL-NATIVE_BUILD_DIR)/.built
	...
	install -d $(CROSSTOOL-NATIVE_IPK_DIR)/opt/$(GNU_TARGET_NAME)/$(CROSS_CONFIGURATION)
	( cd /opt/$(GNU_TARGET_NAME)/$(CROSS_CONFIGURATION) ; tar cf - . ) | \
		( cd $(CROSSTOOL-NATIVE_IPK_DIR)/opt/$(GNU_TARGET_NAME)/$(CROSS_CONFIGURATION) ; tar xvf - )
