...
PERL-CRYPT-OPENSSL-RSA_IPK_VERSION=4
...
$(PERL-CRYPT-OPENSSL-RSA_BUILD_DIR)/.configured: $(DL_DIR)/$(PERL-CRYPT-OPENSSL-RSA_SOURCE) $(PERL-CRYPT-OPENSSL-RSA_PATCHES) make/perl-crypt-openssl-rsa.mk
	$(MAKE) openssl-stage
...
$(PERL-CRYPT-OPENSSL-RSA_IPK): $(PERL-CRYPT-OPENSSL-RSA_BUILD_DIR)/.built
	...
	find $(PERL-CRYPT-OPENSSL-RSA_IPK_DIR)$(TARGET_PREFIX) -name 'perllocal.pod' -exec rm -f {} \;
	(cd $(PERL-CRYPT-OPENSSL-RSA_IPK_DIR)$(TARGET_PREFIX)/lib/perl5 ; \
		find . -name '*.so' -exec chmod +w {} \; ; \
		find . -name '*.so' -exec $(STRIP_COMMAND) {} \; ; \
		find . -name '*.so' -exec chmod -w {} \; ; \
	)
	find $(PERL-CRYPT-OPENSSL-RSA_IPK_DIR)$(TARGET_PREFIX) -type d -exec chmod go+rx {} \;
