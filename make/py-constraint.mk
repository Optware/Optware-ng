...
#PY-CONSTRAINT_CONFFILES=$(TARGET_PREFIX)/etc/py-constraint.conf $(TARGET_PREFIX)/etc/init.d/SXXpy-constraint
...
#
# Binaries should be installed into $(PY-CONSTRAINT_IPK_DIR)$(TARGET_PREFIX)/sbin or $(PY-CONSTRAINT_IPK_DIR)$(TARGET_PREFIX)/bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(PY-CONSTRAINT_IPK_DIR)$(TARGET_PREFIX)/{lib,include}
# Configuration files should be installed in $(PY-CONSTRAINT_IPK_DIR)$(TARGET_PREFIX)/etc/py-constraint/...
# Documentation files should be installed in $(PY-CONSTRAINT_IPK_DIR)$(TARGET_PREFIX)/doc/py-constraint/...
# Daemon startup scripts should be installed in $(PY-CONSTRAINT_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/S??py-constraint
#
...
#	echo $(PY-CONSTRAINT_CONFFILES) | sed -e 's/ /\n/g' > $(PY25-CONSTRAINT_IPK_DIR)/CONTROL/conffiles
	...
	# py-constraint-doc
	$(INSTALL) -d $(PY-CONSTRAINT-DOC_IPK_DIR)$(TARGET_PREFIX)/share/doc/py-constraint/
	$(INSTALL) $(PY-CONSTRAINT_BUILD_DIR)/2.5/README $(PY-CONSTRAINT-DOC_IPK_DIR)$(TARGET_PREFIX)/share/doc/py-constraint/
	cp -a $(PY-CONSTRAINT_BUILD_DIR)/2.5/examples $(PY-CONSTRAINT-DOC_IPK_DIR)$(TARGET_PREFIX)/share/doc/py-constraint/
