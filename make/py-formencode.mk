...
#PY-FORMENCODE_CONFFILES=$(TARGET_PREFIX)/etc/py-formencode.conf $(TARGET_PREFIX)/etc/init.d/SXXpy-formencode
...
#
# Binaries should be installed into $(PY-FORMENCODE_IPK_DIR)$(TARGET_PREFIX)/sbin or $(PY-FORMENCODE_IPK_DIR)$(TARGET_PREFIX)/bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(PY-FORMENCODE_IPK_DIR)$(TARGET_PREFIX)/{lib,include}
# Configuration files should be installed in $(PY-FORMENCODE_IPK_DIR)$(TARGET_PREFIX)/etc/py-formencode/...
# Documentation files should be installed in $(PY-FORMENCODE_IPK_DIR)$(TARGET_PREFIX)/doc/py-formencode/...
# Daemon startup scripts should be installed in $(PY-FORMENCODE_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/S??py-formencode
#
...
$(PY24-FORMENCODE_IPK): $(PY-FORMENCODE_BUILD_DIR)/.built
	...
	PYTHONPATH=$(STAGING_LIB_DIR)/python2.4/site-packages \
	$(HOST_STAGING_PREFIX)/bin/python2.4 setup.py install --root=$(PY24-FORMENCODE_IPK_DIR) --prefix=$(TARGET_PREFIX))
...
$(PY25-FORMENCODE_IPK): $(PY-FORMENCODE_BUILD_DIR)/.built
	...
	PYTHONPATH=$(STAGING_LIB_DIR)/python2.5/site-packages \
	$(HOST_STAGING_PREFIX)/bin/python2.5 setup.py install --root=$(PY25-FORMENCODE_IPK_DIR) --prefix=$(TARGET_PREFIX))
...
$(PY26-FORMENCODE_IPK): $(PY-FORMENCODE_BUILD_DIR)/.built
	...
	PYTHONPATH=$(STAGING_LIB_DIR)/python2.6/site-packages \
	$(HOST_STAGING_PREFIX)/bin/python2.6 setup.py install --root=$(PY26-FORMENCODE_IPK_DIR) --prefix=$(TARGET_PREFIX))
