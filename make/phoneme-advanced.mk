...
#PHONEME_ADVANCED_CONFFILES=$(TARGET_PREFIX)/etc/phoneme-advanced.conf $(TARGET_PREFIX)/etc/init.d/SXXphoneme-advanced
...
ifeq ($(OPTWARE_TARGET), $(filter shibby-tomato-arm buildroot-armeabi buildroot-armeabi-ng buildroot-armv5eabi-ng buildroot-armeabihf buildroot-mipsel, $(OPTWARE_TARGET)))
...
#
# Binaries should be installed into $(PHONEME_ADVANCED_IPK_DIR)$(TARGET_PREFIX)/sbin or $(PHONEME_ADVANCED_IPK_DIR)$(TARGET_PREFIX)/bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(PHONEME_ADVANCED_IPK_DIR)$(TARGET_PREFIX)/{lib,include}
# Configuration files should be installed in $(PHONEME_ADVANCED_IPK_DIR)$(TARGET_PREFIX)/etc/phoneme-advanced/...
# Documentation files should be installed in $(PHONEME_ADVANCED_IPK_DIR)$(TARGET_PREFIX)/doc/phoneme-advanced/...
# Daemon startup scripts should be installed in $(PHONEME_ADVANCED_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/S??phoneme-advanced
#
...
$(PHONEME_ADVANCED_IPK): $(PHONEME_ADVANCED_BUILD_DIR)/.built
	...
	$(INSTALL) -d $(PHONEME_ADVANCED_IPK_DIR)$(TARGET_PREFIX)/lib/java
	cd $(PHONEME_ADVANCED_IPK_DIR)$(TARGET_PREFIX)/lib/java && \
	unzip $(PHONEME_ADVANCED_BUILD_DIR)/cdc/install/phoneme-advanced.zip
	$(STRIP_COMMAND) $(PHONEME_ADVANCED_IPK_DIR)$(TARGET_PREFIX)/lib/java/phoneme-advanced/bin/cvm
	$(INSTALL) -d $(PHONEME_ADVANCED_IPK_DIR)$(TARGET_PREFIX)/bin
	cd $(PHONEME_ADVANCED_IPK_DIR)$(TARGET_PREFIX)/bin; ln -s ../lib/java/phoneme-advanced/bin/cvm .
