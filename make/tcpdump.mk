...
#TCPDUMP_CONFFILES=$(TARGET_PREFIX)/etc/tcpdump.conf $(TARGET_PREFIX)/etc/init.d/SXXtcpdump
...
#
# Binaries should be installed into $(TCPDUMP_IPK_DIR)$(TARGET_PREFIX)/sbin or $(TCPDUMP_IPK_DIR)$(TARGET_PREFIX)/bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(TCPDUMP_IPK_DIR)$(TARGET_PREFIX)/{lib,include}
# Configuration files should be installed in $(TCPDUMP_IPK_DIR)$(TARGET_PREFIX)/etc/tcpdump/...
# Documentation files should be installed in $(TCPDUMP_IPK_DIR)$(TARGET_PREFIX)/doc/tcpdump/...
# Daemon startup scripts should be installed in $(TCPDUMP_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/S??tcpdump
#
...
$(TCPDUMP_IPK): $(TCPDUMP_BUILD_DIR)/.built
	...
	rm -f $(TCPDUMP_IPK_DIR)$(TARGET_PREFIX)/sbin/tcpdump.$(TCPDUMP_VERSION)
	$(STRIP_COMMAND) $(TCPDUMP_IPK_DIR)$(TARGET_PREFIX)/sbin/tcpdump
#	$(INSTALL) -d $(TCPDUMP_IPK_DIR)$(TARGET_PREFIX)/etc/
#	$(INSTALL) -m 644 $(TCPDUMP_SOURCE_DIR)/tcpdump.conf $(TCPDUMP_IPK_DIR)$(TARGET_PREFIX)/etc/tcpdump.conf
#	$(INSTALL) -d $(TCPDUMP_IPK_DIR)$(TARGET_PREFIX)/etc/init.d
#	$(INSTALL) -m 755 $(TCPDUMP_SOURCE_DIR)/rc.tcpdump $(TCPDUMP_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/SXXtcpdump
#	sed -i -e '/^#!/aOPTWARE_TARGET=${OPTWARE_TARGET}' $(TCPDUMP_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/SXXtcpdump
