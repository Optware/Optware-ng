...
#JIKES_CONFFILES=$(TARGET_PREFIX)/etc/jikes.conf $(TARGET_PREFIX)/etc/init.d/SXXjikes
...
#
# Binaries should be installed into $(JIKES_IPK_DIR)$(TARGET_PREFIX)/sbin or $(JIKES_IPK_DIR)$(TARGET_PREFIX)/bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(JIKES_IPK_DIR)$(TARGET_PREFIX)/{lib,include}
# Configuration files should be installed in $(JIKES_IPK_DIR)$(TARGET_PREFIX)/etc/jikes/...
# Documentation files should be installed in $(JIKES_IPK_DIR)$(TARGET_PREFIX)/doc/jikes/...
# Daemon startup scripts should be installed in $(JIKES_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/S??jikes
#
...
$(JIKES_IPK): $(JIKES_BUILD_DIR)/.built
	...
	$(STRIP_COMMAND) $(JIKES_IPK_DIR)$(TARGET_PREFIX)/bin/jikes
#	$(INSTALL) -d $(JIKES_IPK_DIR)$(TARGET_PREFIX)/etc/
#	$(INSTALL) -m 755 $(JIKES_SOURCE_DIR)/jikes.conf $(JIKES_IPK_DIR)$(TARGET_PREFIX)/etc/jikes.conf
#	$(INSTALL) -d $(JIKES_IPK_DIR)$(TARGET_PREFIX)/etc/init.d
#	$(INSTALL) -m 755 $(JIKES_SOURCE_DIR)/rc.jikes $(JIKES_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/SXXjikes
