...
PERL-IO-STRINGY_SITE=http://$(PERL_CPAN_SITE)/CPAN/authors/id/D/DS/DSKOLL
...
$(DL_DIR)/$(PERL-IO-STRINGY_SOURCE):
	$(WGET) -P $(@D) $(PERL-IO-STRINGY_SITE)/$(@F) || \
	$(WGET) -P $(@D) $(FREEBSD_DISTFILES)/$(@F) || \
	$(WGET) -P $(@D) $(SOURCES_NLO_SITE)/$(@F)

perl-io-stringy-source: $(DL_DIR)/$(PERL-IO-STRINGY_SOURCE) $(PERL-IO-STRINGY_PATCHES)

$(PERL-IO-STRINGY_BUILD_DIR)/.configured: $(DL_DIR)/$(PERL-IO-STRINGY_SOURCE) $(PERL-IO-STRINGY_PATCHES) make/perl-io-stringy.mk
	$(MAKE) perl-stage
...
$(PERL-IO-STRINGY_IPK): $(PERL-IO-STRINGY_BUILD_DIR)/.built
	...
	find $(PERL-IO-STRINGY_IPK_DIR)$(TARGET_PREFIX) -name 'perllocal.pod' -exec rm -f {} \;
	(cd $(PERL-IO-STRINGY_IPK_DIR)$(TARGET_PREFIX)/lib/perl5 ; \
		find . -name '*.so' -exec chmod +w {} \; ; \
		find . -name '*.so' -exec $(STRIP_COMMAND) {} \; ; \
		find . -name '*.so' -exec chmod -w {} \; ; \
	)
	find $(PERL-IO-STRINGY_IPK_DIR)$(TARGET_PREFIX) -type d -exec chmod go+rx {} \;
