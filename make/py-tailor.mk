...
#PY-TAILOR_CONFFILES=$(TARGET_PREFIX)/etc/py-tailor.conf $(TARGET_PREFIX)/etc/init.d/SXXpy-tailor
...
#
# Binaries should be installed into $(PY-TAILOR_IPK_DIR)$(TARGET_PREFIX)/sbin or $(PY-TAILOR_IPK_DIR)$(TARGET_PREFIX)/bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(PY-TAILOR_IPK_DIR)$(TARGET_PREFIX)/{lib,include}
# Configuration files should be installed in $(PY-TAILOR_IPK_DIR)$(TARGET_PREFIX)/etc/py-tailor/...
# Documentation files should be installed in $(PY-TAILOR_IPK_DIR)$(TARGET_PREFIX)/doc/py-tailor/...
# Daemon startup scripts should be installed in $(PY-TAILOR_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/S??py-tailor
#
...
$(PY25-TAILOR_IPK): $(PY-TAILOR_BUILD_DIR)/.built
	...
	(cd $(PY-TAILOR_BUILD_DIR)/2.5; \
	    ...
	    --root=$(PY25-TAILOR_IPK_DIR) --prefix=$(TARGET_PREFIX); \
	)
#	-$(STRIP_COMMAND) `find $(PY25-TAILOR_IPK_DIR)$(TARGET_PREFIX)/lib/python2.5/site-packages/tailor -name '*.so'`
...
$(PY26-TAILOR_IPK): $(PY-TAILOR_BUILD_DIR)/.built
	...
	(cd $(PY-TAILOR_BUILD_DIR)/2.6; \
	    ...
	    --root=$(PY26-TAILOR_IPK_DIR) --prefix=$(TARGET_PREFIX); \
	)
	mv $(PY26-TAILOR_IPK_DIR)$(TARGET_PREFIX)/bin/tailor $(PY26-TAILOR_IPK_DIR)$(TARGET_PREFIX)/bin/tailor-2.6
#	-$(STRIP_COMMAND) `find $(PY26-TAILOR_IPK_DIR)$(TARGET_PREFIX)/lib/python2.6/site-packages/tailor -name '*.so'`
