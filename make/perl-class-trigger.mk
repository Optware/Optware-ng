...
PERL-CLASS-TRIGGER_SITE=http://$(PERL_CPAN_SITE)/CPAN/authors/id/M/MI/MIYAGAWA
...
PERL-CLASS-TRIGGER_IPK_VERSION=4
...
$(DL_DIR)/$(PERL-CLASS-TRIGGER_SOURCE):
	$(WGET) -P $(@D) $(PERL-CLASS-TRIGGER_SITE)/$(@F) || \
	$(WGET) -P $(@D) $(FREEBSD_DISTFILES)/$(@F) || \
	$(WGET) -P $(@D) $(SOURCES_NLO_SITE)/$(@F)

perl-class-trigger-source: $(DL_DIR)/$(PERL-CLASS-TRIGGER_SOURCE) $(PERL-CLASS-TRIGGER_PATCHES)

$(PERL-CLASS-TRIGGER_BUILD_DIR)/.configured: $(DL_DIR)/$(PERL-CLASS-TRIGGER_SOURCE) $(PERL-CLASS-TRIGGER_PATCHES) make/perl-class-trigger.mk
...
$(PERL-CLASS-TRIGGER_IPK): $(PERL-CLASS-TRIGGER_BUILD_DIR)/.built
	...
	find $(PERL-CLASS-TRIGGER_IPK_DIR)$(TARGET_PREFIX) -name 'perllocal.pod' -exec rm -f {} \;
	(cd $(PERL-CLASS-TRIGGER_IPK_DIR)$(TARGET_PREFIX)/lib/perl5 ; \
		find . -name '*.so' -exec chmod +w {} \; ; \
		find . -name '*.so' -exec $(STRIP_COMMAND) {} \; ; \
		find . -name '*.so' -exec chmod -w {} \; ; \
	)
	find $(PERL-CLASS-TRIGGER_IPK_DIR)$(TARGET_PREFIX) -type d -exec chmod go+rx {} \;
