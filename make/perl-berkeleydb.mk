...
PERL-BERKELEYDB_CFLAGS=-pthread -fwrapv -fno-strict-aliasing -fstack-protector-strong -D_LARGEFILE_SOURCE -D_FILE_OFFSET_BITS=64
PERL-BERKELEYDB_LDFLAGS=-pthread -fstack-protector-strong
...
$(PERL-BERKELEYDB_BUILD_DIR)/.built: $(PERL-BERKELEYDB_BUILD_DIR)/.configured
	...
	$(MAKE) -C $(@D) \
		$(TARGET_CONFIGURE_OPTS) \
		LD=$(TARGET_CC) \
		CCFLAGS="$(STAGING_CPPFLAGS) $(PERL-BERKELEYDB_CFLAGS)" \
		LDFLAGS="$(STAGING_LDFLAGS) $(PERL-BERKELEYDB_LDFLAGS)" \
		LDDLFLAGS="-shared $(STAGING_LDFLAGS) $(PERL-BERKELEYDB_LDFLAGS)"
		$(PERL_INC) \
	PERL5LIB="$(STAGING_LIB_DIR)/perl5/site_perl"
	touch $@
