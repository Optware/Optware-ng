...
#DS101-KERNEL-MODULES_CONFFILES=$(TARGET_PREFIX)/etc/ds101-kernel-modules.conf $(TARGET_PREFIX)/etc/init.d/SXXds101-kernel-modules
...
$(DS101-KERNEL-MODULES_IPK_DIR)/CONTROL/control:
	for m in $(DS101-KERNEL-MODULES); do \
	  ...
	  echo "echo running $(TARGET_PREFIX)/etc/init.d/S01mod_$$m" >> $(DS101-KERNEL-MODULES_IPK_DIR)-$$m/CONTROL/postinst ;\
	  echo "$(TARGET_PREFIX)/etc/init.d/S01mod_$$m" >> $(DS101-KERNEL-MODULES_IPK_DIR)-$$m/CONTROL/postinst ;\
...
#
# Binaries should be installed into $(DS101-KERNEL-MODULES_IPK_DIR)$(TARGET_PREFIX)/sbin or $(DS101-KERNEL-MODULES_IPK_DIR)$(TARGET_PREFIX)/bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(DS101-KERNEL-MODULES_IPK_DIR)$(TARGET_PREFIX)/{lib,include}
# Configuration files should be installed in $(DS101-KERNEL-MODULES_IPK_DIR)$(TARGET_PREFIX)/etc/ds101-kernel-modules/...
# Documentation files should be installed in $(DS101-KERNEL-MODULES_IPK_DIR)$(TARGET_PREFIX)/doc/ds101-kernel-modules/...
# Daemon startup scripts should be installed in $(DS101-KERNEL-MODULES_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/S??ds101-kernel-modules
#
...
$(DS101-KERNEL-MODULES_BUILD_DIR)/.ipkdone: $(DS101-KERNEL-MODULES_BUILD_DIR)/.built $(DS101-KERNEL-MODULES_SCRIPTS)
	...
	for m in $(DS101-KERNEL-MODULES); do \
	  $(INSTALL) -d $(DS101-KERNEL-MODULES_IPK_DIR)-$$m$(TARGET_PREFIX)/lib/modules; \
	  $(INSTALL) -m 644 `find $(DS101-KERNEL-MODULES_IPK_DIR) -name $$m.o` $(DS101-KERNEL-MODULES_IPK_DIR)-$$m$(TARGET_PREFIX)/lib/modules; \
	  $(INSTALL) -d $(DS101-KERNEL-MODULES_IPK_DIR)-$$m$(TARGET_PREFIX)/etc/init.d; \
	  if [ -f $(DS101-KERNEL-MODULES_SOURCE_DIR)/S01mod_$$m ] ; then \
		$(INSTALL) -m 755  $(DS101-KERNEL-MODULES_SOURCE_DIR)/S01mod_$$m $(DS101-KERNEL-MODULES_IPK_DIR)-$$m$(TARGET_PREFIX)/etc/init.d; \
	  else 	install -m 755  $(DS101-KERNEL-MODULES_SOURCE_DIR)/S01mod_generic $(DS101-KERNEL-MODULES_IPK_DIR)-$$m$(TARGET_PREFIX)/etc/init.d/S01mod_$$m; \
	  fi;\
	done
