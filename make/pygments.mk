...
#PYGMENTS_CONFFILES=$(TARGET_PREFIX)/etc/pygments.conf $(TARGET_PREFIX)/etc/init.d/SXXpygments
...
#
# Binaries should be installed into $(PYGMENTS_IPK_DIR)$(TARGET_PREFIX)/sbin or $(PYGMENTS_IPK_DIR)$(TARGET_PREFIX)/bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(PYGMENTS_IPK_DIR)$(TARGET_PREFIX)/{lib,include}
# Configuration files should be installed in $(PYGMENTS_IPK_DIR)$(TARGET_PREFIX)/etc/pygments/...
# Documentation files should be installed in $(PYGMENTS_IPK_DIR)$(TARGET_PREFIX)/doc/pygments/...
# Daemon startup scripts should be installed in $(PYGMENTS_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/S??pygments
#
...
$(PY25-PYGMENTS_IPK): $(PYGMENTS_BUILD_DIR)/.built
	...
	(cd $(PYGMENTS_BUILD_DIR)/2.5; \
	    $(HOST_STAGING_PREFIX)/bin/python2.5 setup.py install --root=$(PY25-PYGMENTS_IPK_DIR) --prefix=$(TARGET_PREFIX); \
	)
#	$(STRIP_COMMAND) $(PY25-PYGMENTS_IPK_DIR)$(TARGET_PREFIX)/lib/python2.5/site-packages/pygmentslib/*.so
...
$(PY26-PYGMENTS_IPK): $(PYGMENTS_BUILD_DIR)/.built
	...
	(cd $(PYGMENTS_BUILD_DIR)/2.6; \
	    $(HOST_STAGING_PREFIX)/bin/python2.6 setup.py install --root=$(PY26-PYGMENTS_IPK_DIR) --prefix=$(TARGET_PREFIX); \
	)
#	$(STRIP_COMMAND) $(PY26-PYGMENTS_IPK_DIR)$(TARGET_PREFIX)/lib/python2.6/site-packages/pygmentslib/*.so
	for f in $(PY26-PYGMENTS_IPK_DIR)$(TARGET_PREFIX)/*bin/*; \
		do mv $$f `echo $$f | sed 's|$$|-2.6|'`; done
	rm -rf $(PY26-PYGMENTS_IPK_DIR)$(TARGET_PREFIX)/man
