...
$(PERL-JSON-XS_IPK): $(PERL-JSON-XS_BUILD_DIR)/.built
	...
	find $(PERL-JSON-XS_IPK_DIR)$(TARGET_PREFIX) -name 'perllocal.pod' -exec rm -f {} \;
	(cd $(PERL-JSON-XS_IPK_DIR)$(TARGET_PREFIX)/lib/perl5 ; \
		find . -name '*.so' -exec chmod +w {} \; ; \
		find . -name '*.so' -exec $(STRIP_COMMAND) {} \; ; \
		find . -name '*.so' -exec chmod -w {} \; ; \
	)
	find $(PERL-JSON-XS_IPK_DIR)$(TARGET_PREFIX) -type d -exec chmod go+rx {} \;
