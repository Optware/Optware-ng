...
DRRAW_CONFFILES=$(TARGET_PREFIX)/etc/drraw.conf
...
#
# Binaries should be installed into $(DRRAW_IPK_DIR)$(TARGET_PREFIX)/sbin or $(DRRAW_IPK_DIR)$(TARGET_PREFIX)/bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(DRRAW_IPK_DIR)$(TARGET_PREFIX)/{lib,include}
# Configuration files should be installed in $(DRRAW_IPK_DIR)$(TARGET_PREFIX)/etc/drraw/...
# Documentation files should be installed in $(DRRAW_IPK_DIR)$(TARGET_PREFIX)/doc/drraw/...
# Daemon startup scripts should be installed in $(DRRAW_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/S??drraw
#
# You may need to patch your application to make it use these locations.
#
$(DRRAW_IPK): $(DRRAW_BUILD_DIR)/.built
	rm -rf $(DRRAW_IPK_DIR) $(BUILD_DIR)/drraw_*_$(TARGET_ARCH).ipk
	$(INSTALL) -d $(DRRAW_IPK_DIR)$(TARGET_PREFIX)/share/www/cgi-bin
	$(INSTALL) -m 644 $(DRRAW_BUILD_DIR)/drraw.cgi $(DRRAW_IPK_DIR)$(TARGET_PREFIX)/share/www/cgi-bin/drraw.cgi
ifeq ($(OPTWARE_TARGET), fsg3v4)
	$(INSTALL) -d $(DRRAW_IPK_DIR)/var/www/cgi-bin
	ln -s $(TARGET_PREFIX)/share/www/cgi-bin/drraw.cgi $(DRRAW_IPK_DIR)/var/www/cgi-bin/drraw.cgi
endif
	$(INSTALL) -d $(DRRAW_IPK_DIR)$(TARGET_PREFIX)/etc/
	$(INSTALL) -m 644 $(DRRAW_SOURCE_DIR)/drraw.conf $(DRRAW_IPK_DIR)$(TARGET_PREFIX)/etc/drraw.conf
