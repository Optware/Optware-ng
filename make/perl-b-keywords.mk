...
$(PERL-B-KEYWORDS_BUILD_DIR)/.configured: $(DL_DIR)/$(PERL-B-KEYWORDS_SOURCE) $(PERL-B-KEYWORDS_PATCHES) make/perl-b-keywords.mk
	$(MAKE) perl-stage
...
$(PERL-B-KEYWORDS_IPK): $(PERL-B-KEYWORDS_BUILD_DIR)/.built
	...
	find $(PERL-B-KEYWORDS_IPK_DIR)$(TARGET_PREFIX) -name 'perllocal.pod' -exec rm -f {} \;
	(cd $(PERL-B-KEYWORDS_IPK_DIR)$(TARGET_PREFIX)/lib/perl5 ; \
		find . -name '*.so' -exec chmod +w {} \; ; \
		find . -name '*.so' -exec $(STRIP_COMMAND) {} \; ; \
		find . -name '*.so' -exec chmod -w {} \; ; \
	)
	find $(PERL-B-KEYWORDS_IPK_DIR)$(TARGET_PREFIX) -type d -exec chmod go+rx {} \;
