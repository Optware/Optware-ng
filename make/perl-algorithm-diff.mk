...
PERL-ALGORITHM-DIFF_SITE=http://$(PERL_CPAN_SITE)/CPAN/authors/id/T/TY/TYEMQ
...
PERL-ALGORITHM-DIFF_IPK_VERSION=4
...
$(DL_DIR)/$(PERL-ALGORITHM-DIFF_SOURCE):
	$(WGET) -P $(@D) $(PERL-ALGORITHM-DIFF_SITE)/$(@F) || \
	$(WGET) -P $(@D) $(FREEBSD_DISTFILES)/$(@F) || \
	$(WGET) -P $(@D) $(SOURCES_NLO_SITE)/$(@F)

perl-algorithm-diff-source: $(DL_DIR)/$(PERL-ALGORITHM-DIFF_SOURCE) $(PERL-ALGORITHM-DIFF_PATCHES)

$(PERL-ALGORITHM-DIFF_BUILD_DIR)/.configured: $(DL_DIR)/$(PERL-ALGORITHM-DIFF_SOURCE) $(PERL-ALGORITHM-DIFF_PATCHES) make/perl-algorithm-diff.mk
	$(MAKE) perl-stage
...
$(PERL-ALGORITHM-DIFF_IPK): $(PERL-ALGORITHM-DIFF_BUILD_DIR)/.built
	...
	find $(PERL-ALGORITHM-DIFF_IPK_DIR)$(TARGET_PREFIX) -name 'perllocal.pod' -exec rm -f {} \;
	(cd $(PERL-ALGORITHM-DIFF_IPK_DIR)$(TARGET_PREFIX)/lib/perl5 ; \
		find . -name '*.so' -exec chmod +w {} \; ; \
		find . -name '*.so' -exec $(STRIP_COMMAND) {} \; ; \
		find . -name '*.so' -exec chmod -w {} \; ; \
	)
	find $(PERL-ALGORITHM-DIFF_IPK_DIR)$(TARGET_PREFIX) -type d -exec chmod go+rx {} \;
