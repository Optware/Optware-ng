...
PERL-IO-MULTIPLEX_SITE=http://$(PERL_CPAN_SITE)/CPAN/authors/id/B/BB/BBB
...
PERL-IO-MULTIPLEX_IPK_VERSION=3
...
$(DL_DIR)/$(PERL-IO-MULTIPLEX_SOURCE):
	$(WGET) -P $(@D) $(PERL-IO-MULTIPLEX_SITE)/$(@F) || \
	$(WGET) -P $(@D) $(FREEBSD_DISTFILES)/$(@F) || \
	$(WGET) -P $(@D) $(SOURCES_NLO_SITE)/$(@F)

perl-io-multiplex-source: $(DL_DIR)/$(PERL-IO-MULTIPLEX_SOURCE) $(PERL-IO-MULTIPLEX_PATCHES)

$(PERL-IO-MULTIPLEX_BUILD_DIR)/.configured: $(DL_DIR)/$(PERL-IO-MULTIPLEX_SOURCE) $(PERL-IO-MULTIPLEX_PATCHES) make/perl-io-multiplex.mk
...
$(PERL-IO-MULTIPLEX_IPK): $(PERL-IO-MULTIPLEX_BUILD_DIR)/.built
	...
	find $(PERL-IO-MULTIPLEX_IPK_DIR)$(TARGET_PREFIX) -name 'perllocal.pod' -exec rm -f {} \;
	(cd $(PERL-IO-MULTIPLEX_IPK_DIR)$(TARGET_PREFIX)/lib/perl5 ; \
		find . -name '*.so' -exec chmod +w {} \; ; \
		find . -name '*.so' -exec $(STRIP_COMMAND) {} \; ; \
		find . -name '*.so' -exec chmod -w {} \; ; \
	)
	find $(PERL-IO-MULTIPLEX_IPK_DIR)$(TARGET_PREFIX) -type d -exec chmod go+rx {} \;
