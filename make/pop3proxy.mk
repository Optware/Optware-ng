...
POP3PROXY_CONFFILES=$(TARGET_PREFIX)/etc/xinetd.d/pop3proxy
...
#
# Binaries should be installed into $(POP3PROXY_IPK_DIR)$(TARGET_PREFIX)/sbin or $(POP3PROXY_IPK_DIR)$(TARGET_PREFIX)/bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(POP3PROXY_IPK_DIR)$(TARGET_PREFIX)/{lib,include}
# Configuration files should be installed in $(POP3PROXY_IPK_DIR)$(TARGET_PREFIX)/etc/pop3proxy/...
# Documentation files should be installed in $(POP3PROXY_IPK_DIR)$(TARGET_PREFIX)/doc/pop3proxy/...
# Daemon startup scripts should be installed in $(POP3PROXY_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/S??pop3proxy
#
...
#	$(MAKE) -C $(POP3PROXY_BUILD_DIR) DESTDIR=$(POP3PROXY_IPK_DIR) install-strip
	...
	$(INSTALL) -d $(POP3PROXY_IPK_DIR)$(TARGET_PREFIX)/sbin/
	$(INSTALL) -m 755 $(<D)/pop3.proxy $(POP3PROXY_IPK_DIR)$(TARGET_PREFIX)/sbin/
	$(INSTALL) -d $(POP3PROXY_IPK_DIR)$(TARGET_PREFIX)/man/man1/
	$(INSTALL) -m 644 $(<D)/pop3.proxy.1 $(POP3PROXY_IPK_DIR)$(TARGET_PREFIX)/man/man1/
	$(INSTALL) -d $(POP3PROXY_IPK_DIR)$(TARGET_PREFIX)/share/doc/pop3proxy/
	$(INSTALL) -m 644 $(<D)/acp.pop3 $(<D)/LICENSE $(<D)/pop3proxy.lsm \
		$(<D)/README $(POP3PROXY_IPK_DIR)$(TARGET_PREFIX)/share/doc/pop3proxy/
	$(INSTALL) -m 644 $(POP3PROXY_SOURCE_DIR)/pop3proxy.txt $(POP3PROXY_IPK_DIR)$(TARGET_PREFIX)/share/doc/pop3proxy/
	$(INSTALL) -d $(POP3PROXY_IPK_DIR)$(TARGET_PREFIX)/etc/xinetd.d/
	$(INSTALL) -m 644 $(POP3PROXY_SOURCE_DIR)/pop3proxy $(POP3PROXY_IPK_DIR)$(TARGET_PREFIX)/etc/xinetd.d/pop3proxy
	$(INSTALL) -d $(POP3PROXY_IPK_DIR)$(TARGET_PREFIX)/var/pop3proxy/
#	$(INSTALL) -d $(POP3PROXY_IPK_DIR)$(TARGET_PREFIX)/etc/
#	$(INSTALL) -m 644 $(POP3PROXY_SOURCE_DIR)/pop3proxy.conf $(POP3PROXY_IPK_DIR)$(TARGET_PREFIX)/etc/pop3proxy.conf
#	$(INSTALL) -d $(POP3PROXY_IPK_DIR)$(TARGET_PREFIX)/etc/init.d
#	$(INSTALL) -m 755 $(POP3PROXY_SOURCE_DIR)/rc.pop3proxy $(POP3PROXY_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/SXXpop3proxy
#	sed -i -e '/^#!/aOPTWARE_TARGET=${OPTWARE_TARGET}' $(POP3PROXY_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/SXXpop3proxy
