...
#
PY-PSYCOPG2_SITE=http://pkgs.fedoraproject.org/repo/pkgs/python-psycopg2/$(PY-PSYCOPG2_SOURCE)/9f86f726f9aa7f6c4b05d729cad1e70a
...
PY25-PSYCOPG2_DEPENDS=python25, postgresql
PY26-PSYCOPG2_DEPENDS=python26, postgresql
...
#PY-PSYCOPG2_CONFFILES=$(TARGET_PREFIX)/etc/py-psycopg2.conf $(TARGET_PREFIX)/etc/init.d/SXXpy-psycopg2
...
$(PY-PSYCOPG2_BUILD_DIR)/.built: $(PY-PSYCOPG2_BUILD_DIR)/.configured
	rm -f $@
	(cd $(@D)/2.5; \
	 CC='$(TARGET_CC)' LDSHARED='$(TARGET_CC) -shared -Wl,-rpath,$(TARGET_PREFIX)/lib' \
...
#
# Binaries should be installed into $(PY-PSYCOPG2_IPK_DIR)$(TARGET_PREFIX)/sbin or $(PY-PSYCOPG2_IPK_DIR)$(TARGET_PREFIX)/bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(PY-PSYCOPG2_IPK_DIR)$(TARGET_PREFIX)/{lib,include}
# Configuration files should be installed in $(PY-PSYCOPG2_IPK_DIR)$(TARGET_PREFIX)/etc/py-psycopg2/...
# Documentation files should be installed in $(PY-PSYCOPG2_IPK_DIR)$(TARGET_PREFIX)/doc/py-psycopg2/...
# Daemon startup scripts should be installed in $(PY-PSYCOPG2_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/S??py-psycopg2
#
...
$(PY25-PSYCOPG2_IPK): $(PY-PSYCOPG2_BUILD_DIR)/.built
	...
	PYTHONPATH=$(STAGING_LIB_DIR)/python2.5/site-packages \
	    ...
	    --root=$(PY25-PSYCOPG2_IPK_DIR) --prefix=$(TARGET_PREFIX); \
	)
	$(STRIP_COMMAND) `find $(PY25-PSYCOPG2_IPK_DIR)$(TARGET_PREFIX)/lib -name '*.so'`
...
$(PY26-PSYCOPG2_IPK): $(PY-PSYCOPG2_BUILD_DIR)/.built
	...
	PYTHONPATH=$(STAGING_LIB_DIR)/python2.6/site-packages \
	    ...
	    --root=$(PY26-PSYCOPG2_IPK_DIR) --prefix=$(TARGET_PREFIX); \
	)
	$(STRIP_COMMAND) `find $(PY26-PSYCOPG2_IPK_DIR)$(TARGET_PREFIX)/lib -name '*.so'`
