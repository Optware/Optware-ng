...
$(PERL-IMA-DBI_IPK): $(PERL-IMA-DBI_BUILD_DIR)/.built
	...
	find $(PERL-IMA-DBI_IPK_DIR)$(TARGET_PREFIX) -name 'perllocal.pod' -exec rm -f {} \;
	(cd $(PERL-IMA-DBI_IPK_DIR)$(TARGET_PREFIX)/lib/perl5 ; \
		find . -name '*.so' -exec chmod +w {} \; ; \
		find . -name '*.so' -exec $(STRIP_COMMAND) {} \; ; \
		find . -name '*.so' -exec chmod -w {} \; ; \
	)
	find $(PERL-IMA-DBI_IPK_DIR)$(TARGET_PREFIX) -type d -exec chmod go+rx {} \;
