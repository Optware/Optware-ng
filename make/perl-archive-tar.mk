...
PERL-ARCHIVE-TAR_SITE=http://$(PERL_CPAN_SITE)/CPAN/authors/id/K/KA/KANE
...
$(DL_DIR)/$(PERL-ARCHIVE-TAR_SOURCE):
	$(WGET) -P $(@D) $(PERL-ARCHIVE-TAR_SITE)/$(@F) || \
	$(WGET) -P $(@D) $(FREEBSD_DISTFILES)/$(@F) || \
	$(WGET) -P $(@D) $(SOURCES_NLO_SITE)/$(@F)

perl-archive-tar-source: $(DL_DIR)/$(PERL-ARCHIVE-TAR_SOURCE) $(PERL-ARCHIVE-TAR_PATCHES)

$(PERL-ARCHIVE-TAR_BUILD_DIR)/.configured: $(DL_DIR)/$(PERL-ARCHIVE-TAR_SOURCE) $(PERL-ARCHIVE-TAR_PATCHES) make/perl-archive-tar.mk
...
$(PERL-ARCHIVE-TAR_IPK): $(PERL-ARCHIVE-TAR_BUILD_DIR)/.built
	...
	perl -pi -e 's|$(PERL_HOSTPERL)|$(TARGET_PREFIX)/bin/perl|g' $(PERL-ARCHIVE-TAR_IPK_DIR)/*
	find $(PERL-ARCHIVE-TAR_IPK_DIR)$(TARGET_PREFIX) -name 'perllocal.pod' -exec rm -f {} \;
	(cd $(PERL-ARCHIVE-TAR_IPK_DIR)$(TARGET_PREFIX)/lib/perl5 ; \
		find . -name '*.so' -exec chmod +w {} \; ; \
		find . -name '*.so' -exec $(STRIP_COMMAND) {} \; ; \
		find . -name '*.so' -exec chmod -w {} \; ; \
	)
	find $(PERL-ARCHIVE-TAR_IPK_DIR)$(TARGET_PREFIX) -type d -exec chmod go+rx {} \;
