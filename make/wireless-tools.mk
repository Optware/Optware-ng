...
#WIRELESS-TOOLS_CONFFILES=$(TARGET_PREFIX)/etc/wireless-tools.conf $(TARGET_PREFIX)/etc/init.d/SXXwireless-tools
...
$(WIRELESS-TOOLS_BUILD_DIR)/.built: $(WIRELESS-TOOLS_BUILD_DIR)/.configured
	...
	$(MAKE) -C $(WIRELESS-TOOLS_BUILD_DIR) CC=$(TARGET_CC) PREFIX=$(TARGET_PREFIX) LDFLAGS=-Wl,-rpath=$(TARGET_PREFIX)/lib
...
#
# Binaries should be installed into $(WIRELESS-TOOLS_IPK_DIR)$(TARGET_PREFIX)/sbin or $(WIRELESS-TOOLS_IPK_DIR)$(TARGET_PREFIX)/bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(WIRELESS-TOOLS_IPK_DIR)$(TARGET_PREFIX)/{lib,include}
# Configuration files should be installed in $(WIRELESS-TOOLS_IPK_DIR)$(TARGET_PREFIX)/etc/wireless-tools/...
# Documentation files should be installed in $(WIRELESS-TOOLS_IPK_DIR)$(TARGET_PREFIX)/doc/wireless-tools/...
# Daemon startup scripts should be installed in $(WIRELESS-TOOLS_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/S??wireless-tools
#
...
$(WIRELESS-TOOLS_IPK): $(WIRELESS-TOOLS_BUILD_DIR)/.built
	...
	$(STRIP_COMMAND) $(WIRELESS-TOOLS_IPK_DIR)$(TARGET_PREFIX)/bin/*
	$(STRIP_COMMAND) $(WIRELESS-TOOLS_IPK_DIR)$(TARGET_PREFIX)/sbin/*
	$(STRIP_COMMAND) $(WIRELESS-TOOLS_IPK_DIR)$(TARGET_PREFIX)/lib/*
	#$(INSTALL) -d $(WIRELESS-TOOLS_IPK_DIR)$(TARGET_PREFIX)/etc/
	#$(INSTALL) -m 644 $(WIRELESS-TOOLS_SOURCE_DIR)/wireless-tools.conf $(WIRELESS-TOOLS_IPK_DIR)$(TARGET_PREFIX)/etc/wireless-tools.conf
	#$(INSTALL) -d $(WIRELESS-TOOLS_IPK_DIR)$(TARGET_PREFIX)/etc/init.d
	#$(INSTALL) -m 755 $(WIRELESS-TOOLS_SOURCE_DIR)/rc.wireless-tools $(WIRELESS-TOOLS_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/SXXwireless-tools
