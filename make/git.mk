...
#
GIT_PATCHES=\
$(GIT_SOURCE_DIR)/Makefile.patch \
$(GIT_SOURCE_DIR)/ssh-path-2.6.3.patch \
$(GIT_SOURCE_DIR)/disable_fasthash.patch \

GIT-LITE_PATCHES=\
$(GIT_SOURCE_DIR)/Makefile-1.6.patch \
$(GIT_SOURCE_DIR)/ssh-path.patch \
...
ifneq (,$(filter perl, $(PACKAGES)))
	...
	$(MAKE) -C $(@D) \
		$(TARGET_CONFIGURE_OPTS) \
		CPPFLAGS="$(STAGING_CPPFLAGS) $(GIT_CPPFLAGS)" \
		LDFLAGS="$(STAGING_LDFLAGS) $(GIT_LDFLAGS) -lcrypto" \
		NO_TCLTK=true \
		$$GIT_NSEC \
		$(GIT_MAKE_FLAGS) \
		prefix=$(TARGET_PREFIX) all strip
