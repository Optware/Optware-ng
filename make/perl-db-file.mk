...
$(PERL-DB-FILE_BUILD_DIR)/.configured: $(DL_DIR)/$(PERL-DB-FILE_SOURCE) $(PERL-DB-FILE_PATCHES) make/perl-db-file.mk
...
$(PERL-DB-FILE_IPK): $(PERL-DB-FILE_BUILD_DIR)/.built
	...
	find $(PERL-DB-FILE_IPK_DIR)$(TARGET_PREFIX) -name 'perllocal.pod' -exec rm -f {} \;
	(cd $(PERL-DB-FILE_IPK_DIR)$(TARGET_PREFIX)/lib/perl5 ; \
		find . -name '*.so' -exec chmod +w {} \; ; \
		find . -name '*.so' -exec $(STRIP_COMMAND) {} \; ; \
		find . -name '*.so' -exec chmod -w {} \; ; \
	)
	find $(PERL-DB-FILE_IPK_DIR)$(TARGET_PREFIX) -type d -exec chmod go+rx {} \;
