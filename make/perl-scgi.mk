...
$(PERL-SCGI_BUILD_DIR)/.configured: $(DL_DIR)/$(PERL-SCGI_SOURCE) $(PERL-SCGI_PATCHES) make/perl-scgi.mk
...
$(PERL-SCGI_IPK): $(PERL-SCGI_BUILD_DIR)/.built
	...
	find $(PERL-SCGI_IPK_DIR)$(TARGET_PREFIX) -name 'perllocal.pod' -exec rm -f {} \;
	(cd $(PERL-SCGI_IPK_DIR)$(TARGET_PREFIX)/lib/perl5 ; \
		find . -name '*.so' -exec chmod +w {} \; ; \
		find . -name '*.so' -exec $(STRIP_COMMAND) {} \; ; \
		find . -name '*.so' -exec chmod -w {} \; ; \
	)
	find $(PERL-SCGI_IPK_DIR)$(TARGET_PREFIX) -type d -exec chmod go+rx {} \;
