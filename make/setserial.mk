...
# Otherwise calling $(TARGET_PREFIX)/sbin/setserial raises:
# -sh: $(TARGET_PREFIX)/sbin/setserial: not found
...
#
# Binaries should be installed into $(SETSERIAL_IPK_DIR)$(TARGET_PREFIX)/sbin or $(SETSERIAL_IPK_DIR)$(TARGET_PREFIX)/bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(SETSERIAL_IPK_DIR)$(TARGET_PREFIX)/{lib,include}
# Configuration files should be installed in $(SETSERIAL_IPK_DIR)$(TARGET_PREFIX)/etc/setserial/...
# Documentation files should be installed in $(SETSERIAL_IPK_DIR)$(TARGET_PREFIX)/doc/setserial/...
# Daemon startup scripts should be installed in $(SETSERIAL_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/S??setserial
#
...
# setserial's install rule can't easily be made to follow the rules above
	$(INSTALL) -d $(SETSERIAL_IPK_DIR)$(TARGET_PREFIX)/sbin
	$(INSTALL) -d $(SETSERIAL_IPK_DIR)$(TARGET_PREFIX)/man/man8
	$(INSTALL) -m 755 $(SETSERIAL_BUILD_DIR)/setserial $(SETSERIAL_IPK_DIR)$(TARGET_PREFIX)/sbin
	$(TARGET_STRIP) $(SETSERIAL_IPK_DIR)$(TARGET_PREFIX)/sbin/setserial
	$(INSTALL) $(SETSERIAL_BUILD_DIR)/setserial.8 $(SETSERIAL_IPK_DIR)$(TARGET_PREFIX)/man/man8

#	$(MAKE) -C $(SETSERIAL_BUILD_DIR) DESTDIR=$(SETSERIAL_IPK_DIR)$(TARGET_PREFIX) install
#	$(INSTALL) -d $(SETSERIAL_IPK_DIR)$(TARGET_PREFIX)/etc/
#	$(INSTALL) -m 644 $(SETSERIAL_SOURCE_DIR)/setserial.conf $(SETSERIAL_IPK_DIR)$(TARGET_PREFIX)/etc/setserial.conf
#	$(INSTALL) -d $(SETSERIAL_IPK_DIR)$(TARGET_PREFIX)/etc/init.d
#	$(INSTALL) -m 755 $(SETSERIAL_SOURCE_DIR)/rc.setserial $(SETSERIAL_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/SXXsetserial
