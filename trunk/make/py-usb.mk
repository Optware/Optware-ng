...
PY-USB_IPK_DIR=$(BUILD_DIR)/py-usb-$(PY-USB_VERSION)-ipk
PY-USB_IPK=$(BUILD_DIR)/py-usb_$(PY-USB_VERSION)-$(PY-USB_IPK_VERSION)_$(TARGET_ARCH).ipk
...
$(PY-USB_BUILD_DIR)/.configured: $(DL_DIR)/$(PY-USB_SOURCE) $(PY-USB_PATCHES)
	...
	mv $(BUILD_DIR)/$(PY-USB_DIR) $(PY-USB_BUILD_DIR)
	(cd $(PY-USB_BUILD_DIR); \
	    ( \
                echo "[build_ext]"; \
	...
	touch $(PY-USB_BUILD_DIR)/.configured
...
$(PY-USB_BUILD_DIR)/.built: $(PY-USB_BUILD_DIR)/.configured
	rm -f $(PY-USB_BUILD_DIR)/.built
#	$(MAKE) -C $(PY-USB_BUILD_DIR)
	(cd $(PY-USB_BUILD_DIR); \
	$(TARGET_CONFIGURE_OPTS) LDSHARED='$(TARGET_CC) -shared' \
	PYTHONPATH=$(STAGING_LIB_DIR)/python2.4/site-packages \
	python2.4 setup.py build)
	touch $(PY-USB_BUILD_DIR)/.built
...
#
$(PY-USB_IPK_DIR)/CONTROL/control:
	@install -d $(PY-USB_IPK_DIR)/CONTROL
	@rm -f $@
	@echo "Package: py-usb" >>$@
...
#
$(PY-USB_IPK): $(PY-USB_BUILD_DIR)/.built
	rm -rf $(PY-USB_IPK_DIR) $(BUILD_DIR)/py-usb_*_$(TARGET_ARCH).ipk
	(cd $(PY-USB_BUILD_DIR); \
	PYTHONPATH=$(STAGING_LIB_DIR)/python2.4/site-packages \
	python2.4 -c "import setuptools; execfile('setup.py')" install --root=$(PY-USB_IPK_DIR) --prefix=/opt)
	$(MAKE) $(PY-USB_IPK_DIR)/CONTROL/control
#	echo $(PY-USB_CONFFILES) | sed -e 's/ /\n/g' > $(PY-USB_IPK_DIR)/CONTROL/conffiles
	cd $(BUILD_DIR); $(IPKG_BUILD) $(PY-USB_IPK_DIR)

#
# This is called from the top level makefile to create the IPK file.
#
py-usb-ipk: $(PY-USB_IPK)
...
#
py-usb-dirclean:
	rm -rf $(BUILD_DIR)/$(PY-USB_DIR) $(PY-USB_BUILD_DIR) $(PY-USB_IPK_DIR) $(PY-USB_IPK)
