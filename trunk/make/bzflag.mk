...
BZFLAG_IPK=$(BUILD_DIR)/bzflag_$(BZFLAG_VERSION)-$(BZFLAG_IPK_VERSION)_armeb.ipk
...
$(DL_DIR)/$(BZFLAG_SOURCE):
	$(WGET) -P $(DL_DIR) $(BZFLAG_SITE)/$(BZFLAG_SOURCE)
...
$(BZFLAG_BUILD_DIR)/.configured: $(DL_DIR)/$(BZFLAG_SOURCE) $(BZFLAG_PATCHES)
	$(MAKE) zlib-stage libcurl-stage adns-stage
...
$(BZFLAG_BUILD_DIR)/.built: $(BZFLAG_BUILD_DIR)/.configured
	rm -f $(BZFLAG_BUILD_DIR)/.built
	$(MAKE) -C $(BZFLAG_BUILD_DIR)
	touch $(BZFLAG_BUILD_DIR)/.built
...
$(BZFLAG_BUILD_DIR)/.staged: $(BZFLAG_BUILD_DIR)/.built
	rm -f $(BZFLAG_BUILD_DIR)/.staged
	$(MAKE) -C $(BZFLAG_BUILD_DIR) DESTDIR=$(STAGING_DIR) install
	touch $(BZFLAG_BUILD_DIR)/.staged
