...
then
	...
	echo "src/gz cross http://ipkg.nslu2-linux.org/feeds/optware/cs05q3armel/cross/stable" >/opt/etc/ipkg/cross-feed.conf
fi
if [ ! -e /opt/etc/ipkg/kernel-feed.conf ]
then
	echo "Creating /opt/etc/ipkg/kernel-feed.conf..."
	echo "src/gz kernel http://ipkg.nslu2-linux.org/feeds/optware/mssii/cross/stable" >/opt/etc/ipkg/kernel-feed.conf
fi
